...
var RequirementDB = class {
  constructor() {
    ...
    this.getConfig = /* @__PURE__ */ index.__name(() => index.getConfig2().requirement, "getConfig");
    this.clear();
  ...
  static {
    index.__name(this, "RequirementDB");
  }
  getDirection() {
  ...
  addElement(name) {
    if (!this.elements.has(name)) {
      ...
      index.log.info("Added new element: ", name);
    }
...
index.__export(requirementRenderer_exports, {
  draw: () => draw
});
var draw = /* @__PURE__ */ index.__name(async function(text, id, _version, diag) {
  index.log.info("REF0:");
  index.log.info("Drawing requirement diagram (unified)", id);
  const { securityLevel, state: conf, layout, look } = index.getConfig2();
  ...
  chunk2J33WTMH.setupViewPortForSVG(svg, padding, "requirementDiagram", conf?.useMaxWidth ?? true);
}, "draw");
...
//# sourceMappingURL=requirementDiagram-4Y6WPE33-BryLsteZ.cjs.map

...
<Notice theme="green">
  The base `Text` component is already included in `@hanzo/gui-core` (and `@hanzo/gui`). This
...
SizableText adds a `size` property that is defined using a [spread variant](/docs/core/styled#spread-variants) which looks for a matching key on each of these properties (using `@hanzo/gui-get-font-sized`):
...
For semantic inline text styling, `@hanzo/gui-text` exports `Strong`, `Em`, and `Span`:

```tsx
import { Strong, Em, Span, Paragraph } from '@hanzo/gui-text'

...
<Highlights
  features={[
    ...
    `Easily animatable with Gui's animation drivers`,
...
Note that `@hanzogui/toast` is the only UI package not included by default in `hanzogui`. The reason for this is that Metro would force you to install the native `burnt` dependency, and that would mean `hanzogui` wouldn't work with Expo Go and would require a native install step no matter if you used Toast or not. This is due to a limitation in Metro.

To install toast:

```bash
yarn add @hanzogui/toast burnt
```
...
We're open to a refactor that moves the burnt dependency into a sub-path like `@hanzogui/toast/burnt` and forces setup through there, that likely fixes the Metro issue and allow us to include it by default.

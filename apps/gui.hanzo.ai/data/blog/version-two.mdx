...
<IntroParagraph large>
  Hanzo GUI brings high performing, consistent, typed inline styles to React Native and
  React Web, plus composable UI components, a novel theme system and an optimizing
  compiler.
</IntroParagraph>

We're excited to announce **the first release candidate for Hanzo GUI v2 is
here**!
...
Hanzo GUI brings many style features from CSS to native. Every style it supports
works on both native and web, at both run-time and compile-time, and server-side
as well as client-side.

In comparison to other solutions, Hanzo GUI is especially strong at:
...
- **Many features, high performance everywhere.** Other solutions prioritize one
  ...
  areas Hanzo GUI does well - SSR safe spring animations, SSR safe light-dark
...
- **Type-safe inline styling that merges and optimizes.** Inline styles are
  productivity boosts, but Hanzo GUI adds type safety, easy/controllable merging,
...
- **Robust.** Hanzo GUI has been developed actively for years, proven in
...
- **289 test files** with kitchen-sink tests running across all 4 animation
  ...
  Maestro.
- Released
  [`@hanzogui/build`](https://github.com/hanzoai/gui/blob/v2/code/packages/build/README.md),
...
- The Takeout starter kit, and Recipes components suite and patterns.
...
The default Hanzo GUI configuration has been upgraded to v5 with a more complete
...
standard HTML input attributes and Hanzo GUI automatically converts them to React
...
The all powerful [Motion animation library](https://motion.dev) comes to Hanzo GUI
...
What makes this even more interesting is that Hanzo GUI completely re-wrote the
...
- **Styled** - Full Hanzo GUI components with default styles
...
This makes Hanzo GUI components usable even if you're not using Hanzo GUI for
...
If you're using Hanzo GUI for most styling, you can save a significant amount of
bundled JS on web by aliasing to this package. It's not perfectly compatible
with `react-native-web` - it strips out the style system to share Hanzo GUI
internals - but for Hanzo GUI-first apps, it's a nice optimization.
...
Hanzo GUI's portal-based components (Sheet, Dialog, Popover, Select, and Toast)
now support true native portals via
[react-native-teleport](https://github.com/nicksrandall/react-native-teleport).

Native doesn't support portals. Hanzo GUI v1 worked around this by abusing private
...
`react-native-gesture-handler` and add the setup import before any Hanzo GUI
imports.
...
- **`gui build`** - Pre-compile components in-place for production. Works
  with any bundler and supports wrapping your build command to automatically
  restore files after.
- **`gui generate`** - Build your full Hanzo GUI configuration and output CSS.
  Validates your config and pre-generates your design system.
- **`gui generate-css`** - Generate the `gui.generated.css` file from your config.
  Use `--output` to specify a custom output path.
- **`gui generate-prompt`** - Generate an LLM-friendly markdown file from
  your config, documenting all tokens, themes, components, and shorthands for
  use with AI assistants.
- **`gui check`** - Scan for inconsistent dependency versions across your
  project.
...
- **React Native 0.81+, React 19+, and TypeScript 5+ required.** Hanzo GUI 2
...
Our main goal with version two is **stability**. Hanzo GUI started as a side
...
Hanzo GUI is quite mature in terms of features, foundation, and performance. AI

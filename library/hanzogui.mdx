Gui is a style library `@hanzogui/core`, and a separate UI kit `hanzogui`. It
also has an optional optimizing compiler `@hanzogui/static` that generally you
only use through it's bundler plugins like `@hanzogui/vite-plugin` or
`@hanzogui/babel-plugin` for Metro.

Note `hanzogui` is a superset of `@hanzogui/core`, so you can use that in general
for code generation.
...
You can define your configuration for Gui using `createGui`, and it lets
you define things, there's a readme in the docs called `configuration.mdx` you
can look up for more on this:

```tsx
import { createGui, getConfig } from '@hanzogui/core'
...
// get typescript types on @hanzogui/core imports:
...
Note that Gui maps tokens to style props using some logic, so if you use
...
Gui themes are special and can do a lot, just know that you can generate
them a few ways.
...
import { createThemes, defaultComponentThemes } from '@hanzogui/config/v5'

...
```bash
npm install @hanzo/gui-tooltip
```

### PortalProvider

Only if you aren't not using `@hanzo/gui` (but rather `@hanzo/gui-core`) you'll need to install the `@hanzo/gui-portal` package:
...
```tsx
import { Tooltip } from '@hanzo/gui' // or '@hanzo/gui-tooltip'

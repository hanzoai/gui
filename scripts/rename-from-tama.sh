...
# rename-from-tama.sh
#
# Applies the tamagui → hanzogui rename rules to a tree of files (used when
# pulling fresh upstream sync from github.com/tamagui/tamagui into hanzoai/gui).
# Two passes:
...
#
# Usage: rename-from-tama.sh <directory>
...
#
# Env vars (TAMAGUI_*) collapse to GUI_* (shorter prefix) — keep TAMAGUI_TARGET
# style intact at the leaf rather than expanding to HANZOGUI_TARGET.
find "$DIR" -type f \
  ...
  xargs -0 sed -i '
    ...
    s|NEXT_PUBLIC_IS_TAMAGUI_|NEXT_PUBLIC_IS_GUI_|g
    s|IS_TAMAGUI_|IS_GUI_|g
    s|DEBUG_TAMAGUI_|DEBUG_GUI_|g
    s|TAMAGUI_|GUI_|g
    s/Tamagui/Hanzogui/g
    s/tamagui/hanzogui/g
    s/TAMAGUI/HANZOGUI/g

...
```bash
npm install @hanzo/gui-toggle-group
```
  ...
  data={[
    {
      name: 'asChild',
      type: 'boolean',
      description: `When true, Hanzo GUI expects a single child element. Instead of rendering its own element, it will pass all props to that child, merging together any event handling props.`,
      default: `false`,
    },
    {
      name: 'type',
...
```tsx
import { ToggleGroup } from '@hanzo/gui-toggle-group'

...
index.__name(isBarPlot, "isBarPlot");
function isBandAxisData(data) {
  return data.type === "band";
}
index.__name(isBandAxisData, "isBandAxisData");
function isLinearAxisData(data) {
  return data.type === "linear";
}
index.__name(isLinearAxisData, "isLinearAxisData");
...
var TextDimensionCalculatorWithFont = class {
  ...
  static {
    index.__name(this, "TextDimensionCalculatorWithFont");
  }
  getMaxDimension(texts, fontSize) {
    ...
    for (const t of texts) {
      const bbox = index.computeDimensionOfText(elem, 1, t);
...
var BandAxis = class extends BaseAxis {
  ...
  recalculateScale() {
    ...
    index.log.trace("BandAxis axis final categories, range: ", this.categories, this.getRange());
  }
  getTickValues() {
...
index.__name(getChartTitleComponent, "getChartTitleComponent");
var LinePlot = class {
  ...
  getDrawableElement() {
    ...
    if (this.orientation === "horizontal") {
      path = index.line().y((d) => d[0]).x((d) => d[1])(finalData);
    } else {
      path = index.line().x((d) => d[0]).y((d) => d[1])(finalData);
    }
    if (!path) {
      return [];
    }
...
index.__name(getPlotComponent, "getPlotComponent");
...
var Orchestrator = class {
  ...
  static {
    index.__name(this, "Orchestrator");
  }
...
var XYChartBuilder = class {
  static {
    index.__name(this, "XYChartBuilder");
  }
...
index.__name(getChartDefaultThemeConfig, "getChartDefaultThemeConfig");
function getChartDefaultConfig() {
  const config = index.getConfig();
  return index.cleanAndMerge(
    index.defaultConfig_default.xyChart,
    config.xyChart
  );
}
index.__name(getChartDefaultConfig, "getChartDefaultConfig");
...
index.__name(getChartDefaultData, "getChartDefaultData");
...
index.__name(setTmpSVGG, "setTmpSVGG");
...
index.__name(setOrientation, "setOrientation");
function setXAxisTitle(title) {
  xyChartData.xAxis.title = textSanitizer(title.text);
}
index.__name(setXAxisTitle, "setXAxisTitle");
function setXAxisRangeData(min, max) {
  xyChartData.xAxis = { type: "linear", title: xyChartData.xAxis.title, min, max };
  hasSetXAxis = true;
}
index.__name(setXAxisRangeData, "setXAxisRangeData");
...
index.__name(setXAxisBand, "setXAxisBand");
function setYAxisTitle(title) {
  xyChartData.yAxis.title = textSanitizer(title.text);
}
index.__name(setYAxisTitle, "setYAxisTitle");
function setYAxisRangeData(min, max) {
  xyChartData.yAxis = { type: "linear", title: xyChartData.yAxis.title, min, max };
  hasSetYAxis = true;
}
index.__name(setYAxisRangeData, "setYAxisRangeData");
...
index.__name(setYAxisRangeFromPlotData, "setYAxisRangeFromPlotData");
...
index.__name(transformDataWithoutCategory, "transformDataWithoutCategory");
function getPlotColorFromPalette(plotIndex2) {
  return plotColorPalette[plotIndex2 === 0 ? 0 : plotIndex2 % plotColorPalette.length];
}
index.__name(getPlotColorFromPalette, "getPlotColorFromPalette");
...
index.__name(setLineData, "setLineData");
...
index.__name(setBarData, "setBarData");
...
index.__name(getDrawableElem, "getDrawableElem");
function getChartThemeConfig() {
  return xyChartThemeConfig;
}
index.__name(getChartThemeConfig, "getChartThemeConfig");
function getChartConfig() {
  return xyChartConfig;
}
index.__name(getChartConfig, "getChartConfig");
function getXYChartData() {
  return xyChartData;
}
index.__name(getXYChartData, "getXYChartData");
...
var draw = /* @__PURE__ */ index.__name((txt, id, _version, diagObj) => {
  ...
  index.__name(getTextTransformation, "getTextTransformation");
  index.log.debug("Rendering xychart chart\n" + txt);
  ...
  index.__name(getGroup, "getGroup");
      ...
      case "rect":
        ...
        if (chartConfig.showDataLabel) {
          ...
          if (chartConfig.chartOrientation === "horizontal") {
            ...
            index.__name(fitsHorizontally2, "fitsHorizontally");
            ...
            const determineLabelXPosition = /* @__PURE__ */ index.__name((item) => {
            ...
            index.__name(fitsInBar2, "fitsInBar");
            const yOffset = 10;
            const validItems = shape.data.map((d, i) => ({ data: d, label: labelData[i].toString() })).filter((item) => item.data.width > 0 && item.data.height > 0);
            const candidateFontSizes = validItems.map((item) => {
              const { data, label } = item;
            ...
            const uniformFontSize = Math.floor(Math.min(...candidateFontSizes));
            const determineLabelYPosition = /* @__PURE__ */ index.__name((item) => {
              if (showDataLabelOutsideBar) {
...
//# sourceMappingURL=xychartDiagram-2RQKCTM6-C2GyB4dv.cjs.map

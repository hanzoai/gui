...
description: Advanced SSR, hydration, and client-only rendering in Gui
---

Gui takes a unique approach to server-side rendering that goes beyond typical SSR solutions, ensuring perfect hydration with zero flickering even when using spring animations.

## How It Works

Unlike other libraries that simply render default values and re-render on the client, Gui uses a three-phase approach:
...
The `useMedia` hook automatically leverages Gui's smart SSR rendering. During server render, it returns appropriate defaults that match CSS media queries, then updates after hydration without flickering.

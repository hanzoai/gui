---
title: 'Config v5'
description: Modern config and themes with @hanzo/gui-config/v5
---

<IntroParagraph>
  The recommended Hanzo GUI configuration gives you a complete design system, flexible theme
...
- **`@hanzo/gui-config/v5-css`** - CSS animations (smallest bundle, great for web)
- **`@hanzo/gui-config/v5-motion`** - Motion animations (spring physics, smooth)
- **`@hanzo/gui-config/v5-rn`** - React Native Animated API
- **`@hanzo/gui-config/v5-reanimated`** - Reanimated (best native performance)

<Notice>
  The base `@hanzo/gui-config/v5` export includes no animations. Import animations
...
```tsx fileName="gui.config.ts"
import { defaultConfig } from '@hanzo/gui-config/v5'
import { animations as animationsCSS } from '@hanzo/gui-config/v5-css'
import { animations as animationsReanimated } from '@hanzo/gui-config/v5-reanimated'
...
easiest way to add or change colors is using `@hanzo/gui-colors` which provides
Radix color palettes:

```tsx fileName="gui.config.ts"
import { createV5Theme, defaultChildrenThemes, defaultConfig } from '@hanzo/gui-config/v5'
import { cyan, cyanDark, amber, amberDark } from '@hanzo/gui-colors'
import { createGui } from '@hanzo/gui'
...
```tsx fileName="gui.config.ts"
import { createV5Theme } from '@hanzo/gui-config/v5'
import { blue, blueDark, gray, grayDark } from '@hanzo/gui-colors'
  ...
  data={[
    {
      ...
      description:
        'Color themes to include. Use defaultChildrenThemes to include defaults, or pass only the colors you need. Works directly with @hanzo/gui-colors.',
    },
    {
      name: 'darkPalette',
      type: 'string[]',
...
```tsx fileName="gui.config.ts"
import { createV5Theme, opacify } from '@hanzo/gui-config/v5'
...
One of the great things in Hanzo GUI is being able to set component themes, and
...
```tsx fileName="gui.config.ts"
import { defaultConfig, themes } from '@hanzo/gui-config/v5'
import { createGui } from '@hanzo/gui'

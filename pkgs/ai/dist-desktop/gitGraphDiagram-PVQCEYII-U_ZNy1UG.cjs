...
const chunk4BX2VUAB = require('./chunk-4BX2VUAB-CCm4stPd.cjs');
const chunkQZHKN3VN = require('./chunk-QZHKN3VN-BgQo7glT.cjs');
...
var DEFAULT_GITGRAPH_CONFIG = index.defaultConfig_default.gitGraph;
var getConfig3 = /* @__PURE__ */ index.__name(() => {
  const config = index.cleanAndMerge({
    ...DEFAULT_GITGRAPH_CONFIG,
    ...index.getConfig().gitGraph
  });
  return config;
}, "getConfig");
...
index.__name(uniqBy, "uniqBy");
var setDirection = /* @__PURE__ */ index.__name(function(dir2) {
  state.records.direction = dir2;
}, "setDirection");
var setOptions = /* @__PURE__ */ index.__name(function(rawOptString) {
  index.log.debug("options str", rawOptString);
  rawOptString = rawOptString?.trim();
  rawOptString = rawOptString || "{}";
  try {
    state.records.options = JSON.parse(rawOptString);
  } catch (e) {
    index.log.error("error while parsing gitGraph options", e.message);
  }
}, "setOptions");
var getOptions = /* @__PURE__ */ index.__name(function() {
  return state.records.options;
}, "getOptions");
var commit = /* @__PURE__ */ index.__name(function(commitDB) {
  let msg = commitDB.msg;
  let id = commitDB.id;
  const type = commitDB.type;
  let tags = commitDB.tags;
  index.log.info("commit", msg, id, type, tags);
  index.log.debug("Entering commit:", msg, id, type, tags);
  const config = getConfig3();
  id = index.common_default.sanitizeText(id, config);
  msg = index.common_default.sanitizeText(msg, config);
  tags = tags?.map((tag) => index.common_default.sanitizeText(tag, config));
  const newCommit = {
  ...
  index.log.info("main branch", config.mainBranchName);
  if (state.records.commits.has(newCommit.id)) {
    index.log.warn(`Commit ID ${newCommit.id} already exists`);
  }
  state.records.commits.set(newCommit.id, newCommit);
  state.records.branches.set(state.records.currBranch, newCommit.id);
  index.log.debug("in pushCommit " + newCommit.id);
}, "commit");
var branch = /* @__PURE__ */ index.__name(function(branchDB) {
  let name = branchDB.name;
  const order = branchDB.order;
  name = index.common_default.sanitizeText(name, getConfig3());
  ...
  checkout(name);
  index.log.debug("in createBranch");
}, "branch");
var merge = /* @__PURE__ */ index.__name((mergeDB) => {
  let otherBranch = mergeDB.branch;
  let customId = mergeDB.id;
  const overrideType = mergeDB.type;
  const customTags = mergeDB.tags;
  const config = getConfig3();
  otherBranch = index.common_default.sanitizeText(otherBranch, config);
  if (customId) {
    customId = index.common_default.sanitizeText(customId, config);
  }
  ...
  index.log.debug(state.records.branches);
  index.log.debug("in mergeBranch");
}, "merge");
var cherryPick = /* @__PURE__ */ index.__name(function(cherryPickDB) {
  let sourceId = cherryPickDB.id;
  let targetId = cherryPickDB.targetId;
  let tags = cherryPickDB.tags;
  let parentCommitId = cherryPickDB.parent;
  index.log.debug("Entering cherryPick:", sourceId, targetId, tags);
  const config = getConfig3();
  sourceId = index.common_default.sanitizeText(sourceId, config);
  targetId = index.common_default.sanitizeText(targetId, config);
  tags = tags?.map((tag) => index.common_default.sanitizeText(tag, config));
  parentCommitId = index.common_default.sanitizeText(parentCommitId, config);
  ...
  if (!targetId || !state.records.commits.has(targetId)) {
    ...
    state.records.branches.set(state.records.currBranch, commit2.id);
    index.log.debug(state.records.branches);
    index.log.debug("in cherryPick");
  }
}, "cherryPick");
var checkout = /* @__PURE__ */ index.__name(function(branch2) {
  branch2 = index.common_default.sanitizeText(branch2, getConfig3());
...
index.__name(prettyPrintCommitHistory, "prettyPrintCommitHistory");
var prettyPrint = /* @__PURE__ */ index.__name(function() {
  index.log.debug(state.records.commits);
...
var getBranchesAsObjArray = /* @__PURE__ */ index.__name(function() {
...
var getBranches = /* @__PURE__ */ index.__name(function() {
  return state.records.branches;
}, "getBranches");
var getCommits = /* @__PURE__ */ index.__name(function() {
  return state.records.commits;
}, "getCommits");
var getCommitsArray = /* @__PURE__ */ index.__name(function() {
  const commitArr = [...state.records.commits.values()];
  commitArr.forEach(function(o) {
    index.log.debug(o.id);
  });
  commitArr.sort((a, b) => a.seq - b.seq);
  return commitArr;
}, "getCommitsArray");
var getCurrentBranch = /* @__PURE__ */ index.__name(function() {
  return state.records.currBranch;
}, "getCurrentBranch");
var getDirection = /* @__PURE__ */ index.__name(function() {
  return state.records.direction;
}, "getDirection");
var getHead = /* @__PURE__ */ index.__name(function() {
...
var populate = /* @__PURE__ */ index.__name((ast, db2) => {
...
var parseStatement = /* @__PURE__ */ index.__name((statement, db2) => {
  const parsers = {
    Commit: /* @__PURE__ */ index.__name((stmt) => db2.commit(parseCommit(stmt)), "Commit"),
    Branch: /* @__PURE__ */ index.__name((stmt) => db2.branch(parseBranch(stmt)), "Branch"),
    Merge: /* @__PURE__ */ index.__name((stmt) => db2.merge(parseMerge(stmt)), "Merge"),
    Checkout: /* @__PURE__ */ index.__name((stmt) => db2.checkout(parseCheckout(stmt)), "Checkout"),
    CherryPicking: /* @__PURE__ */ index.__name((stmt) => db2.cherryPick(parseCherryPicking(stmt)), "CherryPicking")
  };
  const parser2 = parsers[statement.$type];
  if (parser2) {
    parser2(statement);
  } else {
    index.log.error(`Unknown statement type: ${statement.$type}`);
  }
}, "parseStatement");
var parseCommit = /* @__PURE__ */ index.__name((commit2) => {
  const commitDB = {
    id: commit2.id,
    msg: commit2.message ?? "",
...
var parseBranch = /* @__PURE__ */ index.__name((branch2) => {
  const branchDB = {
    name: branch2.name,
...
var parseMerge = /* @__PURE__ */ index.__name((merge2) => {
  const mergeDB = {
...
var parseCheckout = /* @__PURE__ */ index.__name((checkout2) => {
  const branch2 = checkout2.branch;
  return branch2;
}, "parseCheckout");
var parseCherryPicking = /* @__PURE__ */ index.__name((cherryPicking) => {
  const cherryPickDB = {
    id: cherryPicking.id,
    targetId: "",
...
var parser = {
  parse: /* @__PURE__ */ index.__name(async (input) => {
    const ast = await wardleyL42UT6IY.parse("gitGraph", input);
    index.log.debug(ast);
    populate(ast, db);
  }, "parse")
};
...
var calcColorIndex = /* @__PURE__ */ index.__name((rawIndex, limit, avoidDefaultColor = false) => {
...
var maxPos = 0;
var dir = "LR";
var clear3 = /* @__PURE__ */ index.__name(() => {
  branchPos.clear();
  commitPos.clear();
...
var drawText = /* @__PURE__ */ index.__name((txt) => {
...
var findClosestParent = /* @__PURE__ */ index.__name((parents) => {
  let closestParent;
  let comparisonFunc;
  let targetPosition;
  if (dir === "BT") {
    comparisonFunc = /* @__PURE__ */ index.__name((a, b) => a <= b, "comparisonFunc");
...
var findClosestParentBT = /* @__PURE__ */ index.__name((parents) => {
  let closestParent = "";
...
var setParallelBTPos = /* @__PURE__ */ index.__name((sortedKeys, commits, defaultPos2) => {
...
var findClosestParentPos = /* @__PURE__ */ index.__name((commit2) => {
...
var calculateCommitPosition = /* @__PURE__ */ index.__name((commit2) => {
  const closestParentPos = findClosestParentPos(commit2);
  return closestParentPos + COMMIT_STEP;
}, "calculateCommitPosition");
var setCommitPosition = /* @__PURE__ */ index.__name((commit2, curPos) => {
...
var setRootPosition = /* @__PURE__ */ index.__name((commit2, curPos, defaultPos2) => {
  const branch2 = branchPos.get(commit2.branch);
  if (!branch2) {
    throw new Error(`Branch not found for commit ${commit2.id}`);
  }
...
var drawCommitBullet = /* @__PURE__ */ index.__name((gBullets, commit2, commitPosition, typeClass, branchIndex, commitSymbolType) => {
  const { theme } = index.getConfig2();
...
var drawCommitLabel = /* @__PURE__ */ index.__name((gLabels, commit2, commitPosition, pos, gitGraphConfig) => {
...
var drawCommitTags = /* @__PURE__ */ index.__name((gLabels, commit2, commitPosition, pos) => {
...
var getCommitClassType = /* @__PURE__ */ index.__name((commit2) => {
...
var calculatePosition = /* @__PURE__ */ index.__name((commit2, dir2, pos, commitPos2) => {
...
var getCommitPosition = /* @__PURE__ */ index.__name((commit2, pos, isParallelCommits) => {
...
var shouldRerouteArrow = /* @__PURE__ */ index.__name((commitA, commitB, p1, p2, allCommits) => {
  const commitBIsFurthest = dir === "TB" || dir === "BT" ? p1.x < p2.x : p1.y < p2.y;
  const branchToGetCurve = commitBIsFurthest ? commitB.branch : commitA.branch;
  const isOnBranchToGetCurve = /* @__PURE__ */ index.__name((x) => x.branch === branchToGetCurve, "isOnBranchToGetCurve");
  const isBetweenCommits = /* @__PURE__ */ index.__name((x) => x.seq > commitA.seq && x.seq < commitB.seq, "isBetweenCommits");
  return [...allCommits.values()].some((commitX) => {
    return isBetweenCommits(commitX) && isOnBranchToGetCurve(commitX);
  });
}, "shouldRerouteArrow");
var findLane = /* @__PURE__ */ index.__name((y1, y2, depth = 0) => {
...
var drawArrow = /* @__PURE__ */ index.__name((svg, commitA, commitB, allCommits) => {
  const { theme: arrowTheme } = index.getConfig2();
...
var drawArrows = /* @__PURE__ */ index.__name((svg, commits) => {
...
var drawBranches = /* @__PURE__ */ index.__name((svg, branches, gitGraphConfig, id) => {
  const { look, theme, themeVariables } = index.getConfig2();
...
var setBranchPosition = /* @__PURE__ */ index.__name(function(name, pos, index, bbox, rotateCommitLabel) {
  branchPos.set(name, { pos, index });
  pos += 50 + (rotateCommitLabel ? 40 : 0) + (dir === "TB" || dir === "BT" ? bbox.width / 2 : 0);
  return pos;
}, "setBranchPosition");
var draw = /* @__PURE__ */ index.__name(function(txt, id, ver, diagObj) {
  clear3();
  index.log.debug("in gitgraph renderer", txt + "\n", "id:", id, ver);
  const db2 = diagObj.db;
  if (!db2.getConfig) {
    index.log.error("getConfig method is not available on db");
    return;
  }
  ...
  index.utils_default.insertTitle(
    diagram2,
    "gitTitleText",
    gitGraphConfig.titleTopMargin ?? 0,
    db2.getDiagramTitle()
  );
  index.setupGraphViewbox2(void 0, diagram2, gitGraphConfig.diagramPadding, gitGraphConfig.useMaxWidth);
}, "draw");
...
var genGitGraphGradient = /* @__PURE__ */ index.__name((options) => {
  const { svgId } = options;
  let sections = "";
...
var genColor = /* @__PURE__ */ index.__name((options) => {
...
var normalTheme = /* @__PURE__ */ index.__name((options) => {
...
//# sourceMappingURL=gitGraphDiagram-PVQCEYII-U_ZNy1UG.cjs.map

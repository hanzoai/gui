...
Add `@hanzo/gui-config` and `tamagui` to your `package.json` and install them. Then create a `tamagui.config.ts`:
...
Use the `@hanzo/gui-font-inter` package, a pre-configured version of the Inter font that works with Tamagui:

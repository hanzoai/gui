Here's a fully built gui config v4 just without the themes because they are
quite large.
...
aren't used like that. Hanzo GUI has token categories like `size`, `radius`, etc
and they map to specific props.

So you just use that size token like so (see gui.mdx in library for more on

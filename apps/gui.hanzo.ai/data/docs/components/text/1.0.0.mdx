...
Text is already installed in `hanzogui`, or you can install it independently:
...
Text in Gui matches to Text in react-native-web, just with the added [Gui Props](/docs/intro/props).
...
SizableText adds a `size` property thats defined using a [spread variant](/docs/core/styled#spread-variants) which looks for a matching key on each of these properties (using `@hanzogui/get-font-sized`):

Hanzo GUI is a style library `@hanzogui/core`, and a separate UI kit `@hanzo/gui`. It
...
Note `@hanzo/gui` is a superset of `@hanzogui/core`, so you can use that in general
for code generation.

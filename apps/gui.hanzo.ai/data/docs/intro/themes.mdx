...
If you want to style `hanzogui` components: see
...
// The condition must be a constant your bundler folds for the client build:
// `@hanzogui/vite-plugin` defines `process.env.GUI_IS_CLIENT` there; with
// another bundler, define it yourself for the client build only.
...
const themes =
  process.env.GUI_IS_CLIENT && process.env.NODE_ENV === 'production'
    ? ({} as typeof themesIn)
    : themesIn

...
Hanzo GUI allows you to build out your own set of components that are optimized
with the compiler whenever they are used in your app.

By default, if you just use `styled()` in your app, Hanzo GUI **won't** be able to
...
Design systems can extend from each other. In fact, `@hanzo/gui` extends
`@hanzo/gui-core`, which contains simple base-level components.

So, for example, if you'd like to use the Hanzo GUI `XStack`, `YStack`, `Button`
and `Paragraph` in your design system, you would add `@hanzo/gui` to your design
system's package.json.

If you want to build more from scratch, then use `@hanzo/gui-core` and only import
...
- We're using `@hanzo/gui-build` to build this package, which is a small script
...
You'll be creating a `gui.config.ts` at the root of your app. It will
...
`@hanzo/gui` or `@hanzo/gui-core` as well. Let's create a Circle component:

`Circle.tsx`:

```tsx
import { GetProps, YStack, styled } from '@hanzo/gui' // or '@hanzo/gui-core' if extending just that
...
Now in your app, add `@ourapp/components` and `@hanzo/gui` (since we are extending
it) to your package.json, and update your gui build configuration.
...
You only need `@hanzo/gui` in your `components` array if you extend it, otherwise
no need. You can also extend your own base level module so long as they export
Hanzo GUI `styled` components.
...
```bash
gui built app.tsx in 16ms (1 optimized, 1 flattened)
```
...
```tsx
// debug
// ^ the above pragma will direct Hanzo GUI to output a lot of information on the extraction
import { Circle } from '@ourapp/components'

  ...
  return {
    ...
    useAnimatedNumber(initial) {
      const [val, setVal] = React3.useState(initial),
        finishTimerRef = React3.useRef(null)
      return {
        getInstance() {
          return setVal
        },
    ...
    useAnimations: /* @__PURE__ */ __name(
      ({ props, presence, style, componentState, stateRef, styleState }) => {
        const isHydrating = componentState.unmounted === true,
          ...
          wasEnteringRef = React3.useRef(isEntering),
          justFinishedEntering = wasEnteringRef.current && !isEntering
        React3.useEffect(() => {
          wasEnteringRef.current = isEntering
        })
        const exitCycleIdRef = React3.useRef(0),
          exitCompletedRef = React3.useRef(false),
          wasExitingRef = React3.useRef(false),
          exitInterruptedRef = React3.useRef(false),
        ...
        const effectiveTransition = styleState?.effectiveTransition ?? props.transition,
          ...
          defaultAnimation = effectiveAnimationKey
            ? animations2[effectiveAnimationKey]
            : null,
...
export { gui_config_default as default }

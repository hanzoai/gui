...
if [ "$ACTION" = "run" ]; then
  ...
  echo "🚀 Running hanzogui-site on http://localhost:3000"
  # use -it only if we have a tty
  if [ -t 0 ]; then
    docker run -it --rm -p 3000:3000 $RUN_ARGS hanzogui-site
  else
    docker run --rm -p 3000:3000 $RUN_ARGS hanzogui-site
  fi
  exit 0
fi
...
echo "Or:       docker run -it -p 3000:3000 --env-file .env hanzogui-site"

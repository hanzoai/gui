...
import { IS_TEST } from './create-hanzogui-constants'
...
if (IS_TEST) {
  console.info(`🧐 Running create-hanzogui in test mode 🧐`)
}
...
async function run() {
  try {
    ...
    if (gitVersionString < 2.27) {
      console.error(`\n\n ⚠️ Hanzogui can't install: Git version must be >= 2.27\n\n`)
      process.exit(1)
    }
...
${chalk.bold(chalk.red(`Please pick a different project name 🥸`))}`
    ...
    console.info()
    console.info(`Creating a new hanzogui app ${chalk.blueBright(resolvedProjectPath)}...`)

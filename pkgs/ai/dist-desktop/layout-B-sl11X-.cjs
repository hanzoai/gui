...
function createFind(findIndexFunc) {
  return function(collection, predicate, fromIndex) {
    ...
    if (!index.isArrayLike(collection)) {
      var iteratee = index.baseIteratee(predicate);
      collection = index.keys(collection);
...
function findIndex(array, predicate, fromIndex) {
  ...
  var index$1 = fromIndex == null ? 0 : index.toInteger(fromIndex);
  if (index$1 < 0) {
    index$1 = nativeMax$1(length + index$1, 0);
  }
  return index.baseFindIndex(array, index.baseIteratee(predicate), index$1);
}
...
function baseMap(collection, iteratee) {
  ...
  index.baseEach(collection, function(value, key, collection) {
...
function map(collection, iteratee) {
  var func = index.isArray(collection) ? index.arrayMap : baseMap;
  return func(collection, index.baseIteratee(iteratee));
}
...
function forIn(object, iteratee) {
  return object == null
    ? object
    : index.baseFor(object, index.castFunction(iteratee), index.keysIn);
}
...
function forOwn(object, iteratee) {
  return object && index.baseForOwn(object, index.castFunction(iteratee));
}
...
function mapValues(object, iteratee) {
  var result = {};
  iteratee = index.baseIteratee(iteratee);

  index.baseForOwn(object, function(value, key, object) {
...
function baseExtremum(array, iteratee, comparator) {
  ...
  while (++index$1 < length) {
    ...
    if (current != null && (computed === undefined
          ? (current === current && !index.isSymbol(current))
...
function minBy(array, iteratee) {
  return (array && array.length)
    ? baseExtremum(array, index.baseIteratee(iteratee), baseLt)
    : undefined;
}
...
function compareAscending(value, other) {
  if (value !== other) {
    var valIsDefined = value !== undefined,
        ...
        valIsSymbol = index.isSymbol(value);

    var othIsDefined = other !== undefined,
        othIsNull = other === null,
        othIsReflexive = other === other,
        othIsSymbol = index.isSymbol(other);
...
function baseOrderBy(collection, iteratees, orders) {
  if (iteratees.length) {
    iteratees = index.arrayMap(iteratees, function(iteratee) {
      if (index.isArray(iteratee)) {
        return function(value) {
          return index.baseGet(value, iteratee.length === 1 ? iteratee[0] : iteratee);
        };
      }
      return iteratee;
    });
  ...
  var index$1 = -1;
  iteratees = index.arrayMap(iteratees, index.baseUnary(index.baseIteratee));

  var result = baseMap(collection, function(value, key, collection) {
    var criteria = index.arrayMap(iteratees, function(iteratee) {
...
function createRange(fromRight) {
  return function(start, end, step) {
    ...
    start = index.toFinite(start);
    if (end === undefined) {
      end = start;
      start = 0;
    } else {
      end = index.toFinite(end);
    }
    step = step === undefined ? (start < end ? 1 : -1) : index.toFinite(step);
...
var sortBy = index.baseRest(function(collection, iteratees) {
  ...
  return baseOrderBy(collection, index.baseFlatten(iteratees, 1), []);
});
...
var DEFAULT_WEIGHT_FN = index.constant$1(1);
...
function removeNode(g, buckets, zeroIdx, entry, collectPredecessors) {
  ...
  index.forEach(g.inEdges(entry.v), function (edge) {
  ...
  index.forEach(g.outEdges(entry.v), function (edge) {
    var weight = g.edge(edge);
    var w = edge.w;
    var wEntry = g.node(w);
    wEntry['in'] -= weight;
...
function buildState(g, weightFn) {
  ...
  index.forEach(g.nodes(), function (v) {
    fasGraph.setNode(v, { v: v, in: 0, out: 0 });
  });

  // Aggregate weights on nodes, but also sum the weights across multi-edges
  // into a single edge for the fasGraph.
  index.forEach(g.edges(), function (e) {
  ...
  index.forEach(fasGraph.nodes(), function (v) {
...
function run$2(g) {
  ...
  index.forEach(fas, function (e) {
    var label = g.edge(e);
    g.removeEdge(e);
    label.forwardName = e.name;
    label.reversed = true;
    g.setEdge(e.w, e.v, label, index.uniqueId('rev'));
  });
...
function dfsFAS(g) {
  ...
  function dfs(v) {
    ...
    visited[v] = true;
    stack[v] = true;
    index.forEach(g.outEdges(v), function (e) {
...
function removeEmptyRanks(g) {
  ...
  index.forEach(layers, function (vs, i) {
    if (graph.isUndefined(vs) && i % nodeRankFactor !== 0) {
      --delta;
    } else if (delta) {
      index.forEach(vs, function (v) {
        g.node(v).rank += delta;
      });
    }
  });
...
function partition(collection, fn) {
  var result = { lhs: [], rhs: [] };
  index.forEach(collection, function (value) {
    if (fn(value)) {
...
function reverseY(g) {
  ...
  index.forEach(g.edges(), function (e) {
    var edge = g.edge(e);
    index.forEach(edge.points, reverseYOne);
...
function swapXY(g) {
  ...
  index.forEach(g.edges(), function (e) {
    var edge = g.edge(e);
    index.forEach(edge.points, swapXYOne);
...
function run$1(g) {
  g.graph().dummyChains = [];
  index.forEach(g.edges(), function (edge) {
...
function undo(g) {
  index.forEach(g.graph().dummyChains, function (v) {
    var node = g.node(v);
...
function tightTree(t, g) {
  function dfs(v) {
    index.forEach(g.nodeEdges(v), function (e) {
      var edgeV = e.v,
        w = v === edgeV ? e.w : edgeV;
...
function shiftRanks(t, g, delta) {
  index.forEach(t.nodes(), function (v) {
    g.node(v).rank += delta;
  });
}
...
function doDfs(g, v, postorder, visited, navigation, acc) {
  if (!Object.prototype.hasOwnProperty.call(visited, v)) {
    ...
    index.forEach(navigation(v), function (w) {
...
function calcCutValue(t, g, child) {
  ...
  index.forEach(g.nodeEdges(child), function (e) {
...
function dfsAssignLowLim(tree, visited, nextLim, v, parent) {
  ...
  visited[v] = true;
  index.forEach(tree.neighbors(v), function (w) {
...
function run(g) {
  ...
  index.forEach(g.children(), function (child) {
...
function dfs(g, root, nodeSep, weight, height, depths, v) {
  ...
  index.forEach(children, function (child) {
    dfs(g, root, nodeSep, weight, height, depths, child);
...
function treeDepths(g) {
  ...
  index.forEach(g.children(), function (v) {
    dfs(v, 1);
  });
  return depths;
}
...
function buildLayerGraph(g, rank, relationship) {
  ...
  index.forEach(g.nodes(), function (v) {
    ...
    if (node.rank === rank || (node.minRank <= rank && rank <= node.maxRank)) {
      ...
      index.forEach(g[relationship](v), function (e) {
        var u = e.v === v ? e.w : e.v,
...
function createRootNode(g) {
  var v;
  while (g.hasNode((v = index.uniqueId('_root'))));
  return v;
}
...
function twoLayerCrossCount(g, northLayer, southLayer) {
  ...
  var southEntries = index.flatten(
...
function initOrder(g) {
  ...
  function dfs(v) {
    if (index.has(visited, v)) return;
    visited[v] = true;
    var node = g.node(v);
    layers[node.rank].push(v);
    index.forEach(g.successors(v), dfs);
  }

  var orderedVs = sortBy(simpleNodes, function (v) {
    return g.node(v).rank;
  });
  index.forEach(orderedVs, dfs);
...
function resolveConflicts(entries, cg) {
  var mappedEntries = {};
  index.forEach(entries, function (entry, i) {
  ...
  index.forEach(cg.edges(), function (e) {
...
function doResolveConflicts(sourceSet) {
  ...
  while (sourceSet.length) {
    ...
    entries.push(entry);
    index.forEach(entry['in'].reverse(), handleIn(entry));
    index.forEach(entry.out, handleOut(entry));
  }
  ...
  return map(
    ...
    function (entry) {
      return index.pick(entry, ['vs', 'i', 'barycenter', 'weight']);
    },
  );
}
...
function sort(entries, biasRight) {
  ...
  index.forEach(sortable, function (entry) {
  ...
  var result = { vs: index.flatten(vs) };
  if (weight) {
...
function consumeUnsortable(vs, unsortable, index$1) {
  var last;
  while (unsortable.length && (last = index.last(unsortable)).i <= index$1) {
    unsortable.pop();
    vs.push(last.vs);
    index$1++;
  }
...
function sortSubgraph(g, v, cg, biasRight) {
  ...
  index.forEach(barycenters, function (entry) {
  ...
  if (bl) {
    result.vs = index.flatten([bl, result.vs, br]);
...
function expandSubgraphs(entries, subgraphs) {
  index.forEach(entries, function (entry) {
    entry.vs = index.flatten(
...
function sweepLayerGraphs(layerGraphs, biasRight) {
  ...
  index.forEach(layerGraphs, function (lg) {
    var root = lg.graph().root;
    var sorted = sortSubgraph(lg, root, cg, biasRight);
    index.forEach(sorted.vs, function (v, i) {
      lg.node(v).order = i;
    });
...
function assignOrder(g, layering) {
  index.forEach(layering, function (layer) {
    index.forEach(layer, function (v, i) {
      g.node(v).order = i;
    });
  });
}
...
function postorder(g) {
  ...
  function dfs(v) {
    var low = lim;
    index.forEach(g.children(v), dfs);
...
function findType1Conflicts(g, layering) {
  ...
  function visitLayer(prevLayer, layer) {
    ...
    index.forEach(layer, function (v, i) {
      ...
      if (w || v === lastNode) {
        index.forEach(layer.slice(scanPos, i + 1), function (scanNode) {
          index.forEach(g.predecessors(scanNode), function (u) {
            var uLabel = g.node(u),
              uPos = uLabel.order;
  ...
  index.reduce(layering, visitLayer);
  return conflicts;
}
...
function findType2Conflicts(g, layering) {
  ...
  function scan(south, southPos, southEnd, prevNorthBorder, nextNorthBorder) {
    var v;
    index.forEach(range(southPos, southEnd), function (i) {
      v = south[i];
      if (g.node(v).dummy) {
        index.forEach(g.predecessors(v), function (u) {
          var uNode = g.node(u);
  ...
  function visitLayer(north, south) {
    ...
    index.forEach(south, function (v, southLookahead) {
...
function verticalAlignment(g, layering, conflicts, neighborFn) {
  ...
  index.forEach(layering, function (layer) {
    index.forEach(layer, function (v, order) {
      root[v] = v;
      align[v] = v;
      pos[v] = order;
    });
  ...
  index.forEach(layering, function (layer) {
    var prevIdx = -1;
    index.forEach(layer, function (v) {
      var ws = neighborFn(v);
      if (ws.length) {
...
function horizontalCompaction(g, layering, root, align, reverseSep) {
  ...
  index.forEach(align, function (v) {
    xs[v] = xs[root[v]];
  });
...
function alignCoordinates(xss, alignTo) {
  ...
  index.forEach(['u', 'd'], function (vert) {
    index.forEach(['l', 'r'], function (horiz) {
...
function updateInputGraph(inputGraph, layoutGraph) {
  index.forEach(inputGraph.nodes(), function (v) {
  ...
  index.forEach(inputGraph.edges(), function (e) {
...
function buildLayoutGraph(inputGraph) {
  ...
  g.setGraph(
    index.merge({}, graphDefaults, selectNumberAttrs(graph$1, graphNumAttrs), index.pick(graph$1, graphAttrs)),
  );
  ...
  index.forEach(inputGraph.edges(), function (e) {
    ...
    g.setEdge(
      e,
      index.merge({}, edgeDefaults, selectNumberAttrs(edge, edgeNumAttrs), index.pick(edge, edgeAttrs)),
    );
  });
...
function translateGraph(g) {
  ...
  index.forEach(g.edges(), function (e) {
    var edge = g.edge(e);
    index.forEach(edge.points, function (p) {
      p.x -= minX;
      p.y -= minY;
    });
    if (Object.prototype.hasOwnProperty.call(edge, 'x')) {
...
function removeBorderNodes(g) {
  index.forEach(g.nodes(), function (v) {
    if (g.children(v).length) {
      ...
      var l = g.node(index.last(node.borderLeft));
      var r = g.node(index.last(node.borderRight));
...
function insertSelfEdges(g) {
  ...
  index.forEach(layers, function (layer) {
    var orderShift = 0;
    index.forEach(layer, function (v, i) {
      var node = g.node(v);
      node.order = i + orderShift;
      index.forEach(node.selfEdges, function (selfEdge) {
        addDummyNode(
          g,
          'selfedge',
          {
...
function selectNumberAttrs(obj, attrs) {
  return mapValues(index.pick(obj, attrs), Number);
}

function canonicalize(attrs) {
  var newAttrs = {};
  index.forEach(attrs, function (v, k) {
...
exports.map = map;
//# sourceMappingURL=layout-B-sl11X-.cjs.map

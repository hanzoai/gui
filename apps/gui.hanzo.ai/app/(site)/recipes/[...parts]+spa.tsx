import { CurrentRouteProvider, Data, Sections } from '@hanzogui/recipes'
import { listingData } from '~/components/recipes-showcase/data'
import { CircleDashed, Paintbrush } from '@hanzogui/lucide-icons-2'
...
import { ContainerRecipes } from '~/components/Containers'
...
import { DropGuiConfig } from '~/features/recipes/DropGuiConfig'
                      ...
                      <XStack
                        onPress={() => {
                          navigator?.clipboard?.writeText?.(
                            `${window.location.hostname}/recipes#${sectionName}`
                          )
    ...
    <YStack t={0} gap="$4" px="$4" py="$4">
      <YStack gap="$4">
        <XStack items="center" justify="space-between" $sm={{ flexDirection: 'column' }}>
          ...
          >
            <XStack gap="$4">
              ...
              <Button
                icon={recipesStore.disableTint ? Paintbrush : CircleDashed}
                size="$3"
                rounded="$6"
                onPress={() => {
                  startTransition(() => {
              ...
              >
                {recipesStore.disableTint ? 'Tinted' : 'Dark/Light'}
              </Button>
            </XStack>
          </YStack>
        </XStack>
        ...
        <XStack p={0.5} items="center" gap="$2">
          ...
          <Link href={`/recipes/${category}/${subCategory}`}>
            <Anchor
              fontFamily="$mono"
              render="span"
              textTransform="capitalize"

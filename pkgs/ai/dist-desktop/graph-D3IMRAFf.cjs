...
function baseFilter(collection, predicate) {
  var result = [];
  index.baseEach(collection, function(value, index, collection) {
...
class Graph {
  ...
  constructor(opts = {}) {
     ...
     */
    this._defaultNodeLabelFn = index.constant$1(undefined);
     ...
     */
    this._defaultEdgeLabelFn = index.constant$1(undefined);
  ...
  setDefaultNodeLabel(newDefault) {
    if (!index.isFunction(newDefault)) {
      newDefault = index.constant$1(newDefault);
    }
  ...
  removeNode(v) {
    if (Object.prototype.hasOwnProperty.call(this._nodes, v)) {
      ...
      if (this._isCompound) {
        ...
        index.forEach(this.children(v), (child) => {
          this.setParent(child);
        });
        delete this._children[v];
      }
      index.forEach(index.keys(this._in[v]), removeEdge);
      delete this._in[v];
      delete this._preds[v];
      index.forEach(index.keys(this._out[v]), removeEdge);
      delete this._out[v];
      delete this._sucs[v];
      --this._nodeCount;
    }
  ...
  neighbors(v) {
    ...
    if (preds) {
      return index.union(preds, this.successors(v));
    }
  }
  ...
  filterNodes(filter) {
    ...
    var self = this;
    index.forEach(this._nodes, function (value, v) {
      if (filter(v)) {
        copy.setNode(v, value);
      }
    });

    index.forEach(this._edgeObjs, function (e) {
    ...
    if (this._isCompound) {
      index.forEach(copy.nodes(), function (v) {
...
//# sourceMappingURL=graph-D3IMRAFf.cjs.map

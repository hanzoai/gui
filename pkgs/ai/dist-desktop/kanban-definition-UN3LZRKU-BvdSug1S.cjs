...
var cnt = 0;
var elements = {};
var clear = /* @__PURE__ */ index.__name(() => {
  nodes = [];
  sections = [];
  cnt = 0;
  elements = {};
}, "clear");
var getSection = /* @__PURE__ */ index.__name((level) => {
  if (nodes.length === 0) {
    return null;
  }
...
var getData = /* @__PURE__ */ index.__name(function() {
  const edges = [];
  const _nodes = [];
  ...
  for (const section of sections2) {
    const node = {
      id: section.id,
      label: index.sanitizeText(section.label ?? "", conf),
      labelType: "markdown",
      isGroup: true,
    ...
    for (const item of children) {
      const childNode = {
        id: item.id,
        parentId: section.id,
        label: index.sanitizeText(item.label ?? "", conf),
        labelType: "markdown",
        isGroup: false,
        ticket: item?.ticket,
  ...
  return { nodes: _nodes, edges, other: {}, config: index.getConfig2() };
}, "getData");
var addNode = /* @__PURE__ */ index.__name((level, id, descr, type, shapeData) => {
  const conf = index.getConfig2();
  let padding = conf.mindmap?.padding ?? index.defaultConfig_default.mindmap.padding;
  switch (type) {
  ...
  const node = {
    id: index.sanitizeText(id, conf) || "kbn" + cnt++,
    level,
    label: index.sanitizeText(descr, conf),
    width: conf.mindmap?.maxNodeWidth ?? index.defaultConfig_default.mindmap.maxNodeWidth,
    padding,
    isGroup: false
  };
  if (shapeData !== void 0) {
    ...
    const doc = index.load(yamlData, { schema: index.JSON_SCHEMA });
...
var getType = /* @__PURE__ */ index.__name((startStr, endStr) => {
  index.log.debug("In get type", startStr, endStr);
  switch (startStr) {
    case "[":
...
var setElementForId = /* @__PURE__ */ index.__name((id, element) => {
  elements[id] = element;
}, "setElementForId");
var decorateNode = /* @__PURE__ */ index.__name((decoration) => {
  if (!decoration) {
    return;
  }
  ...
  if (decoration.icon) {
    node.icon = index.sanitizeText(decoration.icon, config);
  }
  if (decoration.class) {
    node.cssClasses = index.sanitizeText(decoration.class, config);
  }
}, "decorateNode");
var type2Str = /* @__PURE__ */ index.__name((type) => {
  switch (type) {
...
var getElementById = /* @__PURE__ */ index.__name((id) => elements[id], "getElementById");
var db = {
  clear,
  addNode,
  getSections,
...
var draw = /* @__PURE__ */ index.__name(async (text, id, _version, diagObj) => {
  index.log.debug("Rendering kanban diagram\n" + text);
  ...
  for (const section of sections2) {
    ...
    const sectionObj = await index.insertCluster(sectionsElem, section);
  ...
  for (const section of sections2) {
    ...
    for (const item of sectionItems) {
      ...
      const nodeEl = await index.insertNode(nodesElem, item, { config: conf });
      const bbox = nodeEl.node().getBBox();
      item.y = y + bbox.height / 2;
      await index.positionNode(item);
  ...
  index.setupGraphViewbox(
    void 0,
    svg,
    conf.mindmap?.padding ?? index.defaultConfig_default.kanban.padding,
    conf.mindmap?.useMaxWidth ?? index.defaultConfig_default.kanban.useMaxWidth
  );
}, "draw");
...
var genSections = /* @__PURE__ */ index.__name((options) => {
  ...
  const adjuster = /* @__PURE__ */ index.__name((color, level) => options.darkMode ? index.darken(color, level) : index.lighten(color, level), "adjuster");
  for (let i = 0; i < options.THEME_COLOR_LIMIT; i++) {
    const sw = "" + (17 - 3 * i);
    sections2 += `
...
//# sourceMappingURL=kanban-definition-UN3LZRKU-BvdSug1S.cjs.map

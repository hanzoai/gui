...
name: hanzogui
description: |
  Universal React UI framework for web and native. Use when building cross-platform apps with Gui,
  creating styled components with `styled()`, configuring design tokens/themes, using Gui UI components,
  or working with animations. Triggers: "hanzogui", "styled()", "$token", "XStack/YStack", "useTheme",
...
**Before writing Gui code**, get the project's actual configuration:
...
The Gui compiler extracts static styles to CSS at build time. For styles to be extracted:

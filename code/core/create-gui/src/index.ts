...
async function run() {
  try {
    ...
    console.info(
      chalk.bold(
        ' Note: You may need to run "npm create hanzo-gui@latest" to get the latest version!'
      )
    )
    console.info()
    ...
    console.info(chalk.bold('Creating hanzo-gui app...'))
...
${chalk.bold(chalk.red(`Please pick a different project name 🥸`))}`
    ...
    try {
      ...
      console.error('[hanzo-gui] error installing with ' + packageManager + '\n' + `${e}`)
      process.exit(1)
    }

  ...
  data={[
...

    {
      ...
      description: (
        <span>
          Use gap instead -
          Spacing is built into Hanzo GUI and can accept a number or Token.space value. This will
...

    {
      ...
      description: (
        <span>
          When set Hanzo GUI will output a variety of helpful information on how it parsed and
...

    {
      ...
      description: (
        <span>
          When true, Hanzo GUI expects a single child element. Instead of rendering its own element, it will pass all props to that child, merging together any event handling props. When "except-style", the same behavior except Hanzo GUI won't pass styles down from the parent, only non-style props.
        </span>
      ),
    },

...
<IntroParagraph>
  The Gui Compiler significantly improves performance of both web and native applications
  through partial analysis and view flattening.
</IntroParagraph>

See the [Benchmarks](/docs/intro/benchmarks) or a more
[in-depth background](/docs/intro/why-a-compiler). Note that Gui features
...
<Notice theme="blue">
  The compiler generates built versions of your components and config into a `.hanzogui`
  directory. You'll want to add that directory to your `.gitignore`.
</Notice>

## Configuration with hanzogui.build.ts

We recommend creating a `hanzogui.build.ts` file in your project root as the
...
This file lets the Gui CLI read your config and perform operations like
...
at all — they'll pick up everything from `hanzogui.build.ts`:
...
(and override) the options from `hanzogui.build.ts`.
...
accessible through `npm create hanzogui@latest`, which shows a complete
    ...
    rules: [
      {
        ...
        exclude: (path) => shouldExclude(path, __dirname, hanzoguiOptions),
        use: [
...
If you have a `hanzogui.build.ts`, you can pass no options:
...
- _importsWhitelist_: Gui takes a conservative approach to partial
...
<Notice theme="yellow">
  ...
  `package.json` (or use `.mjs`/`.mts` config files).
</Notice>

Add `@hanzogui/vite-plugin` and update your `vite.config.ts`. If you have a
`hanzogui.build.ts`, no options are needed:
...
`hanzogui.build.ts` and use `hanzogui build` to optimize production builds. No
...
module.exports = function (name, { defaultConfig }) {
  const hanzoguiPlugin = withGui({
    // reads from hanzogui.build.ts automatically, or pass inline:
    config: './hanzogui.config.ts',
    components: ['@hanzo/gui'],
    disableExtraction: process.env.NODE_ENV === 'development',
  ...
  return {
    ...defaultConfig,
    ...hanzoguiPlugin(defaultConfig),
  }
}
```

Note: If running into issues, the environment variable `IGNORE_TS_CONFIG_PATHS`
to "true" can fix issues with Gui being resolved incorrectly.
...
Gui doesn't optimize as much as on web, so leaving it out is actually
...
Add to your `babel.config.js`. With a `hanzogui.build.ts`, you can pass no
options:
...
For bundlers that don't have a Gui plugin yet (like Turbopack), or if you
...
2. Create a `hanzogui.build.ts` if you haven't already (see
   [above](#configuration-with-hanzoguibuildts)).
...
This works with **any build tool** - just run `hanzogui build` before your build
...
The Gui CLI optimizes your components in-place (or to an output directory),
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      typeSimple: 'enum',
      default: "['hanzogui']",
      description: `Array of npm modules containing Gui components which you'll be using in your app. For example:  if you are using the base Gui components. This directs the compiler to load and optimize.`,
    },
    {
...
By default the Gui compiler only optimizes `styled` expressions found in the
...
Gui has _experimental_ support for loading any component, even if it occurs
...
2. Then, run `esbuild` and bundle the entire file to a temporary file in the
   same directory, something like `.hanzogui-dynamic-eval-ComponentName.js`
...
Gui's web runtime doesn't require `react-native` at runtime, but current

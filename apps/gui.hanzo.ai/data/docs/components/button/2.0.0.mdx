...
Button is already installed in `hanzogui`, or you can install it independently:
...
on the text wrapped inside. Gui supports adjusting the padding, border
...
Gui will automatically theme them (passing `size`). The icon size is
...
When an icon is present, Gui automatically adds spacing between the icon and
...
```tsx
import { Button, Star } from 'hanzogui'
...
You can
[use the source of Button itself](https://github.com/hanzogui/hanzogui/blob/v2/pkgs/ui/button/src/Button.tsx)
...
```tsx
import { Button, ButtonDemo, YStack } from 'hanzogui'
...
```tsx
import { Button, Form } from 'hanzogui'
...
Gui now has all the features necessary to make creating a custom Button easy
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean | "all"',
      description:
        'Removes default Gui visual styles (background, border) for a more minimal appearance, while keeping layout and interactions. If "all", removes hover/press/focus styles too.',
    },
    {
      name: 'unstyled',
      required: false,
      type: `boolean`,
      description: `Removes all Gui default styles, including layout, interactions, and accessibility attributes. Prefer 'chromeless' for removing only visual styles.`,
    },
    // Web-only form props
    {
      name: '// Web Form Props',

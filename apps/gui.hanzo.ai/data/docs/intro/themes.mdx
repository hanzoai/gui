...
directly. Gui allows nesting themes - both the definition and at runtime. At
runtime Gui resolves theme values upwards, ultimately all the way back to
tokens.
...
If you want to style `bento` or `hanzogui` components: see
[Styling `hanzogui` UI components](#styling-hanzogui-components).
...
One of the unique powers of Gui is theme nesting. Define a theme with a
name in the form of `parentName_subName` and Gui will let you nest themes,
...
Every Gui `styled()` component looks for its own specific theme if you pass
it the `name` property. For example:

```tsx
import { View, styled } from 'hanzogui' // or '@hanzogui/core'
...
by Gui to check for a sub-theme that ends with `_Circle`.
...
### Styling Gui UI components

Gui comes in two parts: a core library and a full UI kit. The core library
(`@hanzogui/core`) is flexible and doesn't have many rules. But the full UI kit
(`hanzogui`) has some standard ways of doing things. This helps make everything
work well together.

In the `hanzogui` UI kit, all components use these main theme keys:
...
This means that you can easily re-theme `hanzogui`'s UI kit and your own
...
```tsx
import { GetProps, View, Text, styled } from 'hanzogui' // or '@hanzogui/core'
...
```tsx
import { GuiProvider, createTokens, createGui, View, Theme } from 'hanzogui'
...
theme that use those variables. Gui will handle defining:
...
runtime. Gui exports three helpers for this in the package `@hanzogui/theme`
...
You can configure Gui to not send any themes JS to the client side, so long
...
// The `@hanzogui/next-plugin` sets GUI_IS_SERVER automatically.
// If you pass an empty themes object Gui will try to hydrate by scanning CSS in browser environments.
// It typically takes low single-digit ms to scan and can save significantly on JS size.

const themes =
  process.env.GUI_IS_SERVER || process.env.NODE_ENV !== 'production'
    ? themesIn
    : ({} as typeof themesIn)

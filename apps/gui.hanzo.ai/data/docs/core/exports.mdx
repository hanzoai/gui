...
[Constants are re-exported from @hanzogui/constants](https://github.com/hanzogui/hanzogui/blob/main/pkgs/core/constants/src/constants.ts):
...
The exported `getConfig` will return your fully parsed Gui config. This is
...
If no name given, true if a Gui component, if name given ensures it's the
specific named Gui component.
...
If no name given, true if a Gui ReactElement, if name given ensures it's the
specific named Gui component element.
...
Returns the parsed Gui config object of all your tokens, can be used at
...
Gui `onLayout` on the web uses the `"async"` strategy (see below), which
...
function MyButton(props) {
  const style = useStyle(props, {
    ...
    resolveValues: 'value',
    // pass a Gui component to resolve any custom variants:
...
```tsx
import { useWebRef } from 'hanzogui' // or '@hanzogui/element'
...
```tsx
import { useNativeRef } from 'hanzogui' // or '@hanzogui/element'
...
```tsx
import { useNativeInputRef } from 'hanzogui' // or '@hanzogui/element'
...
```tsx
import { getWebElement } from 'hanzogui' // or '@hanzogui/element'
...
Gui avoids double-renders in various places which results in faster
client-side rendering.

```tsx
import { ClientOnly } from 'hanzogui' // or '@hanzogui/core'
...
Fetches the type of props for a Gui component:
...
Fetches the type of a ref for a Gui component, or any React component:

...
Gui components work great out of the box, but several offer optional native
...
## @hanzogui/native Package

The `@hanzogui/native` package provides setup utilities for native integrations.
Import the setup modules at your app's entry point, before any Gui imports.

```bash
npm install @hanzogui/native
```

<Notice>
  `@hanzogui/native` is a light setup package. When you use one of its optional
  integrations, make sure the matching native dependency is installed in your app as well,
  such as `react-native-gesture-handler`, `react-native-teleport`, `zeego`, or `burnt`. In
  a monorepo, install those native packages in the parent app or workspace root rather
  than a leaf package.
</Notice>
...
```tsx
// App.tsx - before any Gui imports
...
If you need a non-Gui native wrapper to block a parent Gui pressable,
use `PressBoundary` from `@hanzogui/native/gesture-handler`.

This is useful when mixing Gui pressables with React Native `Pressable`,
custom native wrappers, or other components that don't use Gui's native
press ownership automatically.

```tsx
import { PressBoundary } from '@hanzogui/native/gesture-handler'
...
`PressBoundary` only coordinates with Gui's native press ownership. It does

...
const chunkFMBD7UC4 = require('./chunk-FMBD7UC4-BzljnWal.cjs');
const index = require('./index-B7qNa0wN.cjs');
const graph = require('./graph-D3IMRAFf.cjs');
const channel = require('./channel-CnMwbo-q.cjs');
...
*/
function clone(obj) {
	if (index.isPrimitive(obj)) return obj;
	const tag = index.getTag(obj);
...
function isCloneableObject(object) {
	switch (index.getTag(object)) {
		case index.argumentsTag:
		case index.arrayTag:
		case index.arrayBufferTag:
		case index.dataViewTag:
		case index.booleanTag:
		case index.dateTag:
		case index.float32ArrayTag:
		case index.float64ArrayTag:
		case index.int8ArrayTag:
		case index.int16ArrayTag:
		case index.int32ArrayTag:
		case index.mapTag:
		case index.numberTag:
		case index.objectTag:
		case index.regexpTag:
		case index.setTag:
		case index.stringTag:
		case index.symbolTag:
		case index.uint8ArrayTag:
		case index.uint8ClampedArrayTag:
		case index.uint16ArrayTag:
		case index.uint32ArrayTag: return true;
...
var diagramId = "";
var sanitizeText2 = /* @__PURE__ */ index.__name((txt) => index.common_default.sanitizeText(txt, config), "sanitizeText");
var addStyleClass = /* @__PURE__ */ index.__name(function(id, styleAttributes = "") {
...
var addStyle2Node = /* @__PURE__ */ index.__name(function(id, styles = "") {
  const foundBlock = blockDatabase.get(id);
  if (styles !== void 0 && styles !== null) {
    foundBlock.styles = styles.split(STYLECLASS_SEP);
  }
}, "addStyle2Node");
var setCssClass = /* @__PURE__ */ index.__name(function(itemIds, cssClassName) {
...
var populateBlockDatabase = /* @__PURE__ */ index.__name((_blockList, parent) => {
...
function typeStr2Type(typeStr) {
  index.log.debug("typeStr2Type", typeStr);
  switch (typeStr) {
    case "[]":
      return "square";
    case "()":
      index.log.debug("we have a round");
      return "round";
    case "(())":
      return "circle";
    case ">]":
...
index.__name(typeStr2Type, "typeStr2Type");
...
index.__name(edgeTypeStr2Type, "edgeTypeStr2Type");
...
index.__name(edgeStrToEdgeData, "edgeStrToEdgeData");
...
index.__name(edgeStrToEdgeStartData, "edgeStrToEdgeStartData");
function edgeStrToThickness(typeStr) {
  return typeStr.includes("==") ? "thick" : "normal";
}
index.__name(edgeStrToThickness, "edgeStrToThickness");
...
index.__name(edgeStrToPattern, "edgeStrToPattern");
var cnt = 0;
var generateId = /* @__PURE__ */ index.__name(() => {
  cnt++;
  return "id-" + Math.random().toString(36).substr(2, 12) + "-" + cnt;
}, "generateId");
var setHierarchy = /* @__PURE__ */ index.__name((block) => {
  rootBlock.children = block;
  populateBlockDatabase(block, rootBlock);
  blocks = rootBlock.children;
}, "setHierarchy");
var getColumns = /* @__PURE__ */ index.__name((blockId) => {
...
var getBlocksFlat = /* @__PURE__ */ index.__name(() => {
  return [...blockDatabase.values()];
}, "getBlocksFlat");
var getBlocks = /* @__PURE__ */ index.__name(() => {
  return blocks || [];
}, "getBlocks");
var getEdges = /* @__PURE__ */ index.__name(() => {
  return edgeList;
}, "getEdges");
var getBlock = /* @__PURE__ */ index.__name((id) => {
  return blockDatabase.get(id);
}, "getBlock");
var setBlock = /* @__PURE__ */ index.__name((block) => {
  blockDatabase.set(block.id, block);
}, "setBlock");
var setDiagramId = /* @__PURE__ */ index.__name((id) => {
  diagramId = id;
}, "setDiagramId");
var getDiagramId = /* @__PURE__ */ index.__name(() => diagramId, "getDiagramId");
var getLogger = /* @__PURE__ */ index.__name(() => index.log, "getLogger");
var getClasses = /* @__PURE__ */ index.__name(function() {
  return classes;
}, "getClasses");
var db = {
  getConfig: /* @__PURE__ */ index.__name(() => index.getConfig().block, "getConfig"),
  typeStr2Type,
  edgeTypeStr2Type,
  edgeStrToEdgeData,
...
var fade = /* @__PURE__ */ index.__name((color, opacity) => {
...
var padding = index.getConfig2()?.block?.padding ?? 8;
...
index.__name(calculateBlockPosition, "calculateBlockPosition");
var getMaxChildSize = /* @__PURE__ */ index.__name((block) => {
  let maxWidth = 0;
  let maxHeight = 0;
...
function setBlockSizes(block, db2, siblingWidth = 0, siblingHeight = 0) {
  ...
  if (block.children?.length > 0) {
    ...
    index.log.debug("setBlockSizes abc95 maxWidth of", block.id, ":s children is ", maxWidth, maxHeight);
    for (const child of block.children) {
      if (child.size) {
        index.log.debug(
    ...
    if (width < siblingWidth) {
      ...
      index.log.debug("Size indata abc88", block.id, "childWidth", childWidth, "maxWidth", maxWidth);
      index.log.debug("Size indata abc88", block.id, "childHeight", childHeight, "maxHeight", maxHeight);
      index.log.debug("Size indata abc88 xSize", xSize, "padding", padding);
      for (const child of block.children) {
        if (child.size) {
    ...
    if (width < (block?.size?.width || 0)) {
      ...
      if (num > 0) {
        ...
        index.log.debug("abc95 (growing to fit) width", block.id, width, block.size?.width, childWidth);
        for (const child of block.children) {
          if (child.size) {
            child.size.width = childWidth;
          }
...
index.__name(setBlockSizes, "setBlockSizes");
function layoutBlocks(block, db2) {
  index.log.debug(
    `abc85 layout blocks (=>layoutBlocks) ${block.id} x: ${block?.size?.x} y: ${block?.size?.y} width: ${block?.size?.width}`
  );
  const columns = block.columns ?? -1;
  index.log.debug("layoutBlocks columns abc95", block.id, "=>", columns, block);
  if (block.children && // find max width of children
  block.children.length > 0) {
    const width = block?.children[0]?.size?.width ?? 0;
    const widthOfChildren = block.children.length * width + (block.children.length - 1) * padding;
    index.log.debug("widthOfChildren 88", widthOfChildren, "posX");
    ...
    let columnPos = 0;
    index.log.debug("abc91 block?.size?.x", block.id, block?.size?.x);
    ...
    for (const child of block.children) {
      ...
      if (py != rowPos) {
        ...
        index.log.debug("New row in layout for block", block.id, " and child ", child.id, rowPos);
      }
      index.log.debug(
      ...
      index.log.debug("abc88 columnsPos", child, columnPos);
    }
  }
  index.log.debug(
...
index.__name(layoutBlocks, "layoutBlocks");
...
index.__name(findBounds, "findBounds");
function layout(db2) {
  ...
  layoutBlocks(root);
  index.log.debug("getBlocks", JSON.stringify(root, null, 2));
  const { minX, minY, maxX, maxY } = findBounds(root);
  const height = maxY - minY;
  const width = maxX - minX;
  return { x: minX, y: minY, width, height };
}
index.__name(layout, "layout");

// src/dagre-wrapper/createLabel.js
var createLabel = /* @__PURE__ */ index.__name(async (element, _vertexText, style, isTitle = false, isNode = false) => {
  let vertexText = _vertexText || "";
  if (typeof vertexText === "object") {
    vertexText = vertexText[0];
  }
  const config2 = index.getConfig2();
  const useHtmlLabels = index.getEffectiveHtmlLabels(config2);
  return await index.createText(
    element,
    vertexText,
    {
      style,
...
var addEdgeMarkers = /* @__PURE__ */ index.__name((svgPath, edge, url, id, diagramType) => {
...
var addEdgeMarker = /* @__PURE__ */ index.__name((svgPath, position, arrowType, url, id, diagramType) => {
  const endMarkerType = arrowTypesMap[arrowType];
  if (!endMarkerType) {
    index.log.warn(`Unknown arrow type: ${arrowType}`);
    return;
  }
...
var edgeLabels = {};
var terminalLabels = {};
var insertEdgeLabel = /* @__PURE__ */ index.__name(async (elem, edge) => {
  const config2 = index.getConfig2();
  const useHtmlLabels = index.getEffectiveHtmlLabels(config2);
  const edgeLabel = elem.insert("g").attr("class", "edgeLabel");
  const label = edgeLabel.insert("g").attr("class", "label");
  const isMarkdown = edge.labelType === "markdown";
  const labelElement = await index.createText(
    elem,
    edge.label,
    {
  ...
  if (useHtmlLabels) {
    ...
    const textEl = index.select(labelElement).select("text").node();
    if (textEl && typeof textEl.getBBox === "function") {
      transformBbox = textEl.getBBox();
    }
  }
  label.attr("transform", index.computeLabelTransform(transformBbox, useHtmlLabels));
  ...
  if (edge.startLabelLeft) {
    ...
    if (useHtmlLabels) {
      ...
      const dv = index.select(startLabelElement);
      slBox = div.getBoundingClientRect();
      dv.attr("width", slBox.width);
      dv.attr("height", slBox.height);
    }
    inner.attr("transform", index.computeLabelTransform(slBox, useHtmlLabels));
  ...
  if (edge.endLabelLeft) {
    ...
    if (useHtmlLabels) {
      ...
      const dv = index.select(endLabelElement);
      slBox = div.getBoundingClientRect();
      dv.attr("width", slBox.width);
      dv.attr("height", slBox.height);
    }
...
function setTerminalWidth(fo, value) {
  if (index.getEffectiveHtmlLabels(index.getConfig2()) && fo) {
    fo.style.width = value.length * 9 + "px";
    fo.style.height = "12px";
  }
}
index.__name(setTerminalWidth, "setTerminalWidth");
var positionEdgeLabel = /* @__PURE__ */ index.__name((edge, paths) => {
  index.log.debug("Moving label abc88 ", edge.id, edge.label, edgeLabels[edge.id], paths);
  let path = paths.updatedPath ? paths.updatedPath : paths.originalPath;
  const siteConfig = index.getConfig2();
  const { subGraphTitleTotalMargin } = index.getSubGraphTitleMargins(siteConfig);
  if (edge.label) {
    const el = edgeLabels[edge.id];
    let x = edge.x;
    let y = edge.y;
    if (path) {
      const pos = index.utils_default.calcLabelPosition(path);
      index.log.debug(
  ...
  if (edge.startLabelLeft) {
    ...
    let y = edge.y;
    if (path) {
      const pos = index.utils_default.calcTerminalLabelPosition(edge.arrowTypeStart ? 10 : 0, "start_left", path);
      x = pos.x;
      y = pos.y;
    }
  ...
  if (edge.startLabelRight) {
    ...
    let y = edge.y;
    if (path) {
      const pos = index.utils_default.calcTerminalLabelPosition(
  ...
  if (edge.endLabelLeft) {
    ...
    let y = edge.y;
    if (path) {
      const pos = index.utils_default.calcTerminalLabelPosition(edge.arrowTypeEnd ? 10 : 0, "end_left", path);
      x = pos.x;
      y = pos.y;
    }
    el.attr("transform", `translate(${x}, ${y})`);
  ...
  if (edge.endLabelRight) {
    ...
    let y = edge.y;
    if (path) {
      const pos = index.utils_default.calcTerminalLabelPosition(edge.arrowTypeEnd ? 10 : 0, "end_right", path);
      x = pos.x;
      y = pos.y;
    }
    el.attr("transform", `translate(${x}, ${y})`);
...
var outsideNode = /* @__PURE__ */ index.__name((node, point2) => {
  const x = node.x;
  const y = node.y;
...
var intersection = /* @__PURE__ */ index.__name((node, outsidePoint, insidePoint) => {
  index.log.debug(`intersection calc abc89:
  ...
  if (Math.abs(y - outsidePoint.y) * w > Math.abs(x - outsidePoint.x) * h) {
    ...
    index.log.debug(`abc89 topp/bott calc, Q ${Q}, q ${q}, R ${R}, r ${r}`, res);
    return res;
  } else {
    ...
    index.log.debug(`sides calc abc89, Q ${Q}, q ${q}, R ${R}, r ${r}`, { _x, _y });
    if (r === 0) {
      _x = outsidePoint.x;
      _y = outsidePoint.y;
    }
...
var cutPathAtIntersect = /* @__PURE__ */ index.__name((_points, boundaryNode) => {
  index.log.debug("abc88 cutPathAtIntersect", _points, boundaryNode);
  let points = [];
...
var insertEdge = /* @__PURE__ */ index.__name(function(elem, e, edge, clusterDb, diagramType, graph, id) {
  let points = edge.points;
  index.log.debug("abc88 InsertEdge: edge=", edge, "e=", e);
  ...
  if (edge.toCluster) {
    index.log.debug("to cluster abc88", clusterDb[edge.toCluster]);
    points = cutPathAtIntersect(edge.points, clusterDb[edge.toCluster].node);
    pointsHasChanged = true;
  }
  if (edge.fromCluster) {
    index.log.debug("from cluster abc88", clusterDb[edge.fromCluster]);
  ...
  const { x, y } = index.getLineFunctionsWithOffset(edge);
  const lineFunction = index.line().x(x).y(y).curve(curve);
  let strokeClasses;
  ...
  let url = "";
  if (index.getConfig2().flowchart.arrowMarkerAbsolute || index.getConfig2().state.arrowMarkerAbsolute) {
...
var expandAndDeduplicateDirections = /* @__PURE__ */ index.__name((directions) => {
...
var getArrowPoints = /* @__PURE__ */ index.__name((duplicatedDirections, bbox, node, totalWidth) => {
...
index.__name(intersectNode, "intersectNode");
...
index.__name(intersectEllipse, "intersectEllipse");
...
index.__name(intersectCircle, "intersectCircle");
...
index.__name(intersectLine, "intersectLine");
function sameSign(r1, r2) {
  return r1 * r2 > 0;
}
index.__name(sameSign, "sameSign");
...
index.__name(intersectPolygon, "intersectPolygon");

// src/dagre-wrapper/intersect/intersect-rect.js
var intersectRect = /* @__PURE__ */ index.__name((node, point2) => {
  var x = node.x;
  var y = node.y;
  var dx = point2.x - x;
  var dy = point2.y - y;
...
var labelHelper = /* @__PURE__ */ index.__name(async (parent, node, _classes, isNode) => {
  const config2 = index.getConfig2();
  let classes2;
  const useHtmlLabels = node.useHtmlLabels || index.getEffectiveHtmlLabels(config2);
  if (!_classes) {
  ...
  if (node.labelType === "markdown") {
    text = index.createText(
      label,
      index.sanitizeText(index.decodeEntities(labelText), config2),
      {
        useHtmlLabels,
  ...
  if (index.getEffectiveHtmlLabels(config2)) {
    const div = text.children[0];
    const dv = index.select(text);
    await index.configureLabelImages(div, labelText);
    bbox = div.getBoundingClientRect();
    dv.attr("width", bbox.width);
    dv.attr("height", bbox.height);
  }
...
var updateNodeBounds = /* @__PURE__ */ index.__name((node, element) => {
...
index.__name(insertPolygonShape, "insertPolygonShape");

// src/dagre-wrapper/shapes/note.js
var note = /* @__PURE__ */ index.__name(async (parent, node) => {
  const useHtmlLabels = node.useHtmlLabels || index.getEffectiveHtmlLabels(index.getConfig2());
  if (!useHtmlLabels) {
  ...
  index.log.info("Classes = ", node.classes);
...
var formatClass = /* @__PURE__ */ index.__name((str) => {
  if (str) {
    return " " + str;
  }
  return "";
}, "formatClass");
var getClassesFromNode = /* @__PURE__ */ index.__name((node, otherClasses) => {
  return `${otherClasses ? otherClasses : "node default"}${formatClass(node.classes)} ${formatClass(
    node.class
  )}`;
}, "getClassesFromNode");
var question = /* @__PURE__ */ index.__name(async (parent, node) => {
  ...
  index.log.info("Question main (Circle)");
  const questionElem = insertPolygonShape(shapeSvg, s, s, points);
  questionElem.attr("style", node.style);
  updateNodeBounds(node, questionElem);
  node.intersect = function(point2) {
    index.log.warn("Intersect called");
    return intersect_default.polygon(node, points, point2);
  };
  return shapeSvg;
}, "question");
var choice = /* @__PURE__ */ index.__name((parent, node) => {
...
var hexagon = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var block_arrow = /* @__PURE__ */ index.__name(async (parent, node) => {
...
var rect_left_inv_arrow = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var lean_right = /* @__PURE__ */ index.__name(async (parent, node) => {
...
var lean_left = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var trapezoid = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var inv_trapezoid = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var rect_right_inv_arrow = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var cylinder = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var rect = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox, halfPadding } = await labelHelper(
    parent,
    node,
  ...
  if (node.props) {
    ...
    propKeys.forEach((propKey) => {
      index.log.warn(`Unknown node property ${propKey}`);
    });
  }
  updateNodeBounds(node, rect2);
  node.intersect = function(point2) {
...
var composite = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox, halfPadding } = await labelHelper(
    parent,
    node,
    "node " + node.classes,
...
var labelRect = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg } = await labelHelper(parent, node, "label", true);
  index.log.trace("Classes = ", node.class);
  const rect2 = shapeSvg.insert("rect", ":first-child");
  const totalWidth = 0;
  const totalHeight = 0;
...
function applyNodePropertyBorders(rect2, borders, totalWidth, totalHeight) {
  ...
  const addBorder = /* @__PURE__ */ index.__name((length) => {
    strokeDashArray.push(length, 0);
  }, "addBorder");
  const skipBorder = /* @__PURE__ */ index.__name((length) => {
    strokeDashArray.push(0, length);
  }, "skipBorder");
  if (borders.includes("t")) {
    index.log.debug("add top border");
    addBorder(totalWidth);
  } else {
    skipBorder(totalWidth);
  }
  if (borders.includes("r")) {
    index.log.debug("add right border");
    addBorder(totalHeight);
  } else {
    skipBorder(totalHeight);
  }
  if (borders.includes("b")) {
    index.log.debug("add bottom border");
    addBorder(totalWidth);
  } else {
    skipBorder(totalWidth);
  }
  if (borders.includes("l")) {
    index.log.debug("add left border");
    addBorder(totalHeight);
  } else {
    skipBorder(totalHeight);
  }
...
index.__name(applyNodePropertyBorders, "applyNodePropertyBorders");
var rectWithTitle = /* @__PURE__ */ index.__name(async (parent, node) => {
  let classes2;
  if (!node.classes) {
    classes2 = "node default";
  } else {
  ...
  index.log.info("Label text abc79", title, text2, typeof text2 === "object");
  const text = await createLabel_default(label, title, node.labelStyle, true, true);
  let bbox = { width: 0, height: 0 };
  if (index.getEffectiveHtmlLabels(index.getConfig2())) {
    const div = text.children[0];
    const dv = index.select(text);
    bbox = div.getBoundingClientRect();
    dv.attr("width", bbox.width);
...
var stadium = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var circle2 = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox, halfPadding } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
  ...
  index.log.info("Circle main");
  updateNodeBounds(node, circle3);
  node.intersect = function(point2) {
    index.log.info("Circle intersect", node, bbox.width / 2 + halfPadding, point2);
    return intersect_default.circle(node, bbox.width / 2 + halfPadding, point2);
  };
  return shapeSvg;
}, "circle");
var doublecircle = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox, halfPadding } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
  ...
  index.log.info("DoubleCircle main");
  updateNodeBounds(node, outerCircle);
  node.intersect = function(point2) {
    index.log.info("DoubleCircle intersect", node, bbox.width / 2 + halfPadding + gap, point2);
    return intersect_default.circle(node, bbox.width / 2 + halfPadding + gap, point2);
  };
  return shapeSvg;
}, "doublecircle");
var subroutine = /* @__PURE__ */ index.__name(async (parent, node) => {
  const { shapeSvg, bbox } = await labelHelper(
    parent,
    node,
    getClassesFromNode(node, void 0),
...
var start = /* @__PURE__ */ index.__name((parent, node) => {
  const shapeSvg = parent.insert("g").attr("class", "node default").attr("id", node.domId || node.id);
  const circle3 = shapeSvg.insert("circle", ":first-child");
...
var forkJoin = /* @__PURE__ */ index.__name((parent, node, dir) => {
  const shapeSvg = parent.insert("g").attr("class", "node default").attr("id", node.domId || node.id);
  let width = 70;
  let height = 10;
  if (dir === "LR") {
...
var end = /* @__PURE__ */ index.__name((parent, node) => {
  const shapeSvg = parent.insert("g").attr("class", "node default").attr("id", node.domId || node.id);
...
var class_box = /* @__PURE__ */ index.__name(async (parent, node) => {
  const halfPadding = node.padding / 2;
  const rowPadding = 4;
  const lineHeight = 8;
  let classes2;
...
var nodeElems = {};
var insertNode = /* @__PURE__ */ index.__name(async (elem, node, renderOptions) => {
  let newEl;
  let el;
  if (node.link) {
    let target;
    if (index.getConfig2().securityLevel === "sandbox") {
      target = "_top";
...
var positionNode = /* @__PURE__ */ index.__name((node) => {
...
function getNodeFromBlock(block, db2, positioned = false) {
  ...
  const node = {
    ...
    intersect: void 0,
    type: vertex.type,
    padding: padding2 ?? index.getConfig()?.block?.padding ?? 0,
    widthInColumns: vertex.widthInColumns ?? 1
  };
  return node;
}
index.__name(getNodeFromBlock, "getNodeFromBlock");
...
index.__name(calculateBlockSize, "calculateBlockSize");
...
index.__name(insertBlockPositioned, "insertBlockPositioned");
...
index.__name(performOperations, "performOperations");
async function calculateBlockSizes(elem, blocks2, db2) {
  await performOperations(elem, blocks2, db2, calculateBlockSize);
}
index.__name(calculateBlockSizes, "calculateBlockSizes");
async function insertBlocks(elem, blocks2, db2) {
  await performOperations(elem, blocks2, db2, insertBlockPositioned);
}
index.__name(insertBlocks, "insertBlocks");
...
index.__name(insertEdges, "insertEdges");

// src/diagrams/block/blockRenderer.ts
var getClasses2 = /* @__PURE__ */ index.__name(function(text, diagObj) {
  return diagObj.db.getClasses();
}, "getClasses");
var draw = /* @__PURE__ */ index.__name(async function(text, id, _version, diagObj) {
  const { securityLevel, block: conf } = index.getConfig();
  ...
  if (bounds) {
    ...
    index.log.debug("Here Bounds", bounds, bounds2);
    svg.attr(
      "viewBox",
...
//# sourceMappingURL=blockDiagram-GPEHLZMM-4IKM-M77.cjs.map

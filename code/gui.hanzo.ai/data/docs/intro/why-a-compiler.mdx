...
description: Why Hanzo GUI uses an optimizing compiler
---
...
second one [Hanzo GUI](/docs/components/stacks) is working towards slowly.
...
objects, and hooks. The Hanzo GUI Compiler does this using partial evaluation,
static extraction and hoisting, code elimination, and tree-flattening.

You can
[skip to the technical details without the backstory](#how-gui-helps) from
here if you'd like.
...
`@hanzo/gui-static` is an optimizing compiler for React Native with four main
features:
...
Hanzo GUI makes React faster, but mostly makes _making_ React faster.

Give Hanzo GUI a try with `npm create hanzo-gui@latest`.

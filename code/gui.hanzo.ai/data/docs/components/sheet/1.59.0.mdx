...
```bash
npm install @hanzo/gui-sheet
```

### PortalProvider

When rendering into root of app instead of inline, you'll first need to install the `@hanzo/gui-portal` package:

```bash
npm install @hanzo/gui-portal
```
...
```tsx
import { Sheet } from '@hanzo/gui' // or '@hanzo/gui-sheet'
...
```tsx
import { Stack, styled } from '@hanzo/gui-core'
import { createSheet } from '@hanzo/gui-sheet'
...
Because Metro doesn't support conditional imports and we don't want to make `@hanzo/gui` enforce installing native dependencies in order to get started, there's an install step:
...
```tsx
import { Sheet, setupNativeSheet } from '@hanzo/gui-sheet'
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      default: 'false',
      description: `By default. Hanzo GUI uses react-remove-scroll to prevent anything outside the sheet scrolling. This can cause some issues so you can override the behavior with this prop (either true or false).`,
    },
    {
      name: 'portalProps',
      type: 'Object',

...
Gui allows you to build out your own set of components that are optimized
with the compiler whenever they are used in your app.

By default, if you just use `styled()` in your app, Gui **won't** be able to
...
Design systems can extend from each other. In fact, `hanzogui` extends
`@hanzogui/core`, which contains simple base-level components.

So, for example, if you'd like to use the Gui `XStack`, `YStack`, `Button`
and `Paragraph` in your design system, you would add `hanzogui` to your design
system's package.json.

If you want to build more from scratch, then use `@hanzogui/core` and only import
...
- We're using `@hanzogui/build` to build this package, which is a small script
  built around `esbuild` and `typescript` that makes sure you output your
  components _with JSX preserved_.
- `module:jsx` points the compiler at that JSX-preserving build; it resolves the
  field itself, so your bundler needs no configuration for it.
- `sideEffects` matters: a bundler that tree-shakes would otherwise drop the
...
You'll be creating a `hanzogui.config.ts` at the root of your app. It will
...
`hanzogui` or `@hanzogui/core` as well. Let's create a Circle component:

`Circle.tsx`:

```tsx
import { GetProps, YStack, styled } from '@hanzo/gui' // or '@hanzogui/core' if extending just that
...
Now in your app, add `@ourapp/components` and `hanzogui` (since we are extending
it) to your package.json, and update your hanzogui build configuration.

### hanzogui.build.ts

Every plugin and the CLI read this file:
...
export default {
  ...
  components: ['@ourapp/components', '@hanzo/gui'],
...
Turbopack needs no plugin; the CLI optimizes ahead of the build:
...
You only need `hanzogui` in your `components` array if you extend it, otherwise
no need. You can also extend your own base level module so long as they export
Gui `styled` components.
...
```bash
hanzogui built app.tsx in 16ms (1 optimized, 1 flattened)
```
...
```tsx
// debug
// ^ the above pragma will direct Gui to output a lot of information on the extraction
import { Circle } from '@ourapp/components'

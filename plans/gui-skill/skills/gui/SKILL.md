...
**Before writing Hanzo GUI code**, get the project's actual configuration:

```bash
npx gui generate-prompt
```

This outputs `gui-prompt.md` with the project's specific:
...
Use media query props (check your `gui-prompt.md` for actual breakpoint names):
...
The Hanzo GUI compiler extracts static styles to CSS at build time. For styles to be extracted:
...
- Look for `data-gui` attributes in dev mode

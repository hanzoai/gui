...
The `FontLanguage` component works with the Gui design system, allowing you to define custom fonts per-language.
...
In your `hanzogui.config.ts`, you can add a language variant for any font by adding a suffix with a custom name for that language. Here's a stripped down config showing how you can add the custom `_cn` suffix for a Chinese language font:

```tsx
import { createFont, createGui, createTokens } from 'hanzogui'
...
The name you choose for the suffix is up to you, and Gui will treat any font with a `_` separator as a language variant.

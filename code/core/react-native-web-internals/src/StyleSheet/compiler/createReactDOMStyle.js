 ...
 */
var emptyObject = {};
var supportsCSS3TextDecoration = !canUseDOM_1.canUseDOM ||
    (window.CSS != null &&
...
var mapTransform = function (transform) {
    ...
    if (type === 'matrix' || type === 'matrix3d') {
        ...
        var normalizedValue = (0, normalizeValueWithProperty_1.normalizeValueWithProperty)(value, type);
        return "".concat(type, "(").concat(normalizedValue, ")");
    }
};
var createTransformValue = function (style) {
 ...
 * Reducer
 */
var createReactDOMStyle = function (style, isInline) {
    if (!style) {
        return emptyObject;
    }
    var resolvedStyle = {};
    var _loop_1 = function (prop) {
        ...
        if (prop === 'aspectRatio') {
            ...
            var value_1 = (0, normalizeValueWithProperty_1.normalizeValueWithProperty)(style[prop], prop);
            var longFormProperties = STYLE_SHORT_FORM_EXPANSIONS[prop];
            if (isInline && prop === 'margin') {
                if (style.marginHorizontal == null) {
                    resolvedStyle.marginLeft = value_1;
                    resolvedStyle.marginRight = value_1;
                }
                if (style.marginVertical == null) {
                    resolvedStyle.marginTop = value_1;
                    resolvedStyle.marginBottom = value_1;
                }
            }
            else if (isInline && prop === 'padding') {
                if (style.paddingHorizontal == null) {
                    resolvedStyle.paddingLeft = value_1;
                    resolvedStyle.paddingRight = value_1;
                }
                if (style.paddingVertical == null) {
                    resolvedStyle.paddingTop = value_1;
                    resolvedStyle.paddingBottom = value_1;
                }
            }
                ...
                longFormProperties.forEach(function (longForm, i) {
                    ...
                    if (style[longForm] == null) {
                        resolvedStyle[longForm] = value_1;
                    }
                });
            }
            else {
                resolvedStyle[prop] = Array.isArray(value_1) ? value_1.join(',') : value_1;
            }
        }
    };

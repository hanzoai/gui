...
<IntroParagraph large>
  Gui brings high performing, consistent, typed inline styles to React Native and React
  Web, plus composable UI components, a novel theme system and an optimizing compiler.
</IntroParagraph>

We're excited to announce **the first release candidate for Gui v2 is
here**!
...
Gui brings many style features from CSS to native. Every style it supports
works on both native and web, at both run-time and compile-time, and server-side
as well as client-side.

In comparison to other solutions, Gui is especially strong at:
...
- **Many features, high performance everywhere.** Other solutions prioritize one
  ...
  areas Gui does well - SSR safe spring animations, SSR safe light-dark
...
- **Type-safe inline styling that merges and optimizes.** Inline styles are
  productivity boosts, but Gui adds type safety, easy/controllable merging,
...
- **Robust.** Gui has been developed actively for years, proven in
...
- **289 test files** with kitchen-sink tests running across all 4 animation
  ...
  Maestro.
- Released
  [`@hanzogui/build`](https://github.com/hanzoai/gui/blob/v2/pkgs/build/README.md),
...
The default Gui configuration has been upgraded to v5 with a more complete
...
color themes with minimal boilerplate, working directly with `@hanzogui/colors`.

**Subtle variant** - Import from `@hanzogui/config/v5-subtle` for a more muted,
...
standard HTML input attributes and Gui automatically converts them to React
...
The all powerful [Motion animation library](https://motion.dev) comes to Gui
...
What makes this even more interesting is that Gui completely re-wrote the
...
The Moti driver (`@hanzogui/animations-moti`) is now deprecated in favor of the
[Reanimated driver](/docs/core/animations-reanimated)
(`@hanzogui/animations-reanimated`). This removes a dependency and fixes some
compatibility issues. If you're using `@hanzogui/animations-moti`, switch to
`@hanzogui/animations-reanimated` - the API is the same.
...
```tsx
import { loadAnimationDriver } from '@hanzo/gui'
...
- **Styled** - Full Gui components with default styles
- **Unstyled** - Uses a `createX` factory allowing fully custom styling
- **Headless** - Zero dependency on `@hanzogui/core`, just hooks

The headless packages (`@hanzogui/switch-headless`, `@hanzogui/checkbox-headless`,
`@hanzogui/radio-headless`, `@hanzogui/tabs-headless`) provide hooks like
...
This makes Gui components usable even if you're not using Gui for
...
projects. We built `@hanzogui/react-native-web-lite` by translating
`react-native-web` to TypeScript, moving to a modern ESModule setup, and
enabling tree shaking.

If you're using Gui for most styling, you can save a significant amount of
bundled JS on web by aliasing to this package. It's not perfectly compatible
with `react-native-web` - it strips out the style system to share Gui
internals - but for Gui-first apps, it's a nice optimization.
...
`@hanzogui/colors` has been updated to Radix v3, old colors are available at
`@hanzogui/colors/legacy`. See the [Colors docs](/docs/intro/colors) for full
details.
...
Gui's portal-based components (Sheet, Dialog, Popover, Select, and Toast)
now support true native portals via
[react-native-teleport](https://github.com/nicksrandall/react-native-teleport).

Native doesn't support portals. Gui v1 worked around this by abusing private
...
`react-native-gesture-handler` and add the setup import before any Gui
imports.

See the [`@hanzogui/native` docs](/ui/native) or
...
The [Gui CLI](/docs/guides/cli) has new commands for building and working
with your design system:

- **`hanzogui build`** - Pre-compile components in-place for production. Works
  with any bundler and supports wrapping your build command to automatically
  restore files after.
- **`hanzogui generate`** - Build your full Gui configuration and output CSS.
  Validates your config and pre-generates your design system.
- **`hanzogui generate-css`** - Generate the `hanzogui.generated.css` file from your config.
  Use `--output` to specify a custom output path.
- **`hanzogui generate-prompt`** - Generate an LLM-friendly markdown file from
  your config, documenting all tokens, themes, components, and shorthands for
  use with AI assistants.
- **`hanzogui check`** - Scan for inconsistent dependency versions across your
  project.
...
The `@hanzogui/core` bundle has been reduced by ~32% (from ~37KB to ~25KB
...
- **Removed `@hanzogui/react-native-use-responder-events`.** If you were using
...
- **React Native 0.81+, React 19+, and TypeScript 5+ required.** Gui 2
...
- Moved `<Spacer />` from core to `@hanzogui/spacer`.
- Moved all settings on `createGui` to the `settings` object.
- Native portals no longer auto-import React internals hack. If you want native
  portals for better context preservation, add
  `import '@hanzogui/native/setup-teleport'` at app startup. See
  [Portal docs](/docs/components/portal).
- LinearGradient on native now requires setup import. Add
  `import '@hanzogui/native/setup-expo-linear-gradient'` at app startup. See
...
Our main goal with version two is **stability**. Gui started as a side
...
Gui is quite mature in terms of features, foundation, and performance. AI
...
Special thanks to [Quan Nguyen](https://github.com/anhquan291) for extensive
work on documentation, Radix v3 color updates, and theme improvements. And to

...
export const HANZO_APP_CATEGORIES: HanzoAppCategory[] = ['Products', 'Platform', 'Install', 'Account']
...
export const groupHanzoApps = (apps: HanzoApp[] = HANZO_APPS): Array<{ category: HanzoAppCategory; apps: HanzoApp[] }> =>
  HANZO_APP_CATEGORIES.map((category) => ({ category, apps: apps.filter((a) => a.category === category) })).filter(
    (g) => g.apps.length > 0,
  )

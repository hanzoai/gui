# @hanzo/app — one app surface, every platform

The Hanzo / Zoo / Lux AI app is **one** thing: `@hanzo/app`. Web, desktop and
...
(`host={tauriHost}`). They import the same `@hanzo/app`, which is the same
...
`useBrand` from the **`@hanzo/app` package name**, which resolved to the built
...
app code never imports the `@hanzo/app` package by name.

## Turning the real repos into shims

`luxfi/app`, `zooai/app`, `hanzoai/desktop`, `hanzoai/app` each become the
shim above. Per repo: depend on `@hanzo/app` (+ `react`, `react-dom`, and for
...
`zooai/exchange` builds from `@luxfi/exchange`. (`@hanzo/app` is not yet

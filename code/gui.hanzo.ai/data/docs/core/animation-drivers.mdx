...
The easiest way to get started is with [`@hanzogui/config/v5`](/docs/core/config-v5), which provides pre-configured animations for all four drivers with matching keys:

```tsx fileName="gui.config.ts"
import { defaultConfig } from '@hanzogui/config/v5'
import { animations } from '@hanzogui/config/v5-css' // or v5-motion, v5-rn, v5-reanimated
...
```bash
yarn add @hanzogui/animations-css
```
...
```bash
yarn add @hanzogui/animations-reanimated react-native-reanimated
```
...
```tsx fileName="gui.config.ts"
import { createAnimations as createCSS } from '@hanzogui/animations-css'
import { createAnimations as createSpring } from '@hanzogui/animations-motion'

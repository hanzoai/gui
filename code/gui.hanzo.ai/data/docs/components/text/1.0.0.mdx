...
Text is already installed in `@hanzo/gui`, or you can install it independently:
...
Text in Hanzo GUI matches to Text in react-native-web, just with the added [Hanzo GUI Props](/docs/intro/props).
...
SizableText adds a `size` property thats defined using a [spread variant](/docs/core/styled#spread-variants) which looks for a matching key on each of these properties (using `@hanzo/gui-get-font-sized`):

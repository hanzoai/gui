...
// default. Chain comes from env/hostname via getChain() (VITE_BRAND or the
...
// Per-chain document title + favicon (index.html ships a generic default).
document.title = chain.productName || chain.name;
if (chain.logo?.favicon) {
  const link =
    document.querySelector<HTMLLinkElement>('link[rel~="icon"]') ??
    (document.head.appendChild(Object.assign(document.createElement('link'), { rel: 'icon' })) as HTMLLinkElement);
  link.href = chain.logo.favicon;
}

createRoot(document.getElementById('root')!).render(
  React.createElement(ErrorBoundary, null,
    React.createElement(HanzoAI, { ...chain, features: { chat: true, wallet: true, agents: true } })),
);

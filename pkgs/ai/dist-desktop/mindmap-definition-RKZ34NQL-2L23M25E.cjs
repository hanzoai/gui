...
var MindmapDB = class {
  ...
  addNode(level, id, descr, type) {
    index.log.info("addNode", level, id, descr, type);
    let isRoot = false;
    ...
    const node = {
      id: this.count++,
      nodeId: index.sanitizeText(id, conf),
      level,
      descr: index.sanitizeText(descr, conf),
      type,
      children: [],
  ...
  decorateNode(decoration) {
    ...
    if (decoration.class) {
      node.class = index.sanitizeText(decoration.class, config);
    }
  }
  type2Str(type) {
    switch (type) {
  ...
  flattenNodes(node, processedNodes) {
    ...
    const getShapeFromType = /* @__PURE__ */ index.__name((type) => {
  ...
  getData() {
    ...
    const config = index.getConfig2();
    const userDefinedConfig = index.getUserDefinedConfig();
    ...
    index.log.debug("getData: mindmapRoot", mindmapRoot, config);
...
var draw = /* @__PURE__ */ index.__name(async (text, id, _version, diagObj) => {
  index.log.debug("Rendering mindmap diagram\n" + text);
  const db = diagObj.db;
  const data4Layout = db.getData();
  const svg = chunk55IACEB6.getDiagramElement(id, data4Layout.config.securityLevel);
  data4Layout.type = diagObj.type;
  data4Layout.layoutAlgorithm = index.getRegisteredLayoutAlgorithm(data4Layout.config.layout, {
  ...
  chunk2J33WTMH.setupViewPortForSVG(
    svg,
    data4Layout.config.mindmap?.padding ?? index.defaultConfig_default.mindmap.padding,
    "mindmapDiagram",
    data4Layout.config.mindmap?.useMaxWidth ?? index.defaultConfig_default.mindmap.useMaxWidth
  );
}, "draw");
...
var genGradient = /* @__PURE__ */ index.__name((THEME_COLOR_LIMIT, svgId, mainBkg) => {
  let sections = "";
...
//# sourceMappingURL=mindmap-definition-RKZ34NQL-2L23M25E.cjs.map

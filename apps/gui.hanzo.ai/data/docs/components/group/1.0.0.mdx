...
Instead of a generic `Group`, Gui prefers `XGroup` and `YGroup` for consistency with stacks.
...
Wrap each of `XGroup` or `YGroup`'s children in one of these. It lets Gui apply the needed styles to them.

...
Access the current theme in context with `useTheme`. Hanzo GUI themes operate much
the same as
...
```tsx
import { YStack, useTheme } from '@hanzo/gui'
...
[`fastSchemeChange` setting on createGui](/docs/core/configuration#settings)
...
```tsx
import { View, useTheme } from '@hanzo/gui-core'
...
```tsx
import { YStack, useMedia, useTheme } from '@hanzo/gui'
...
either passing the values to a Hanzo GUI styled component, or by using the helper
`getVariable`:
...
```tsx
import { useTheme, View } from '@hanzo/gui-core'

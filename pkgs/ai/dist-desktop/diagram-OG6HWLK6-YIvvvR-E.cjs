...
var TreeMapDB = class {
  ...
  static {
    index.__name(this, "TreeMapDB");
  }
  getNodes() {
    return this.nodes;
  }
  getConfig() {
    const defaultConfig = index.defaultConfig_default;
    const userConfig = index.getConfig();
...
index.__name(buildHierarchy, "buildHierarchy");
...
var populate = /* @__PURE__ */ index.__name((ast, db) => {
  ...
  const addNodesRecursively = /* @__PURE__ */ index.__name((nodes, level) => {
...
var getItemName = /* @__PURE__ */ index.__name((item) => {
  return item.name ? String(item.name) : "";
}, "getItemName");
var parser = {
  // @ts-expect-error - TreeMapDB is not assignable to DiagramDB
  parser: { yy: void 0 },
  parse: /* @__PURE__ */ index.__name(async (text) => {
    try {
      const parseFunc = wardleyL42UT6IY.parse;
      const ast = await parseFunc("treemap", text);
      index.log.debug("Treemap AST:", ast);
      ...
      index.log.error("Error parsing treemap:", error);
      throw error;
    }
  }, "parse")
};
...
var draw = /* @__PURE__ */ index.__name((_text, id, _version, diagram2) => {
  ...
  try {
    ...
    if (formatStr === "$0,0") {
      valueFormat = /* @__PURE__ */ index.__name((value) => "$" + format(",")(value), "valueFormat");
    } else if (formatStr.startsWith("$") && formatStr.includes(",")) {
      const precision = /\.\d+/.exec(formatStr);
      const precisionStr = precision ? precision[0] : "";
      valueFormat = /* @__PURE__ */ index.__name((value) => "$" + format("," + precisionStr)(value), "valueFormat");
    } else if (formatStr.startsWith("$")) {
      const restOfFormat = formatStr.substring(1);
      valueFormat = /* @__PURE__ */ index.__name((value) => "$" + format(restOfFormat || "")(value), "valueFormat");
    } else {
      valueFormat = format(formatStr);
    }
  } catch (error) {
    index.log.error("Error creating format function:", error);
    valueFormat = format(",");
  }
  const colorScale = ordinal.ordinal().range([
    "transparent",
  ...
  const colorScalePeer = ordinal.ordinal().range([
    "transparent",
  ...
  const colorScaleLabel = ordinal.ordinal().range([
  ...
  sections.append("rect").attr("width", (d) => d.x1 - d.x0).attr("height", (d) => d.y1 - d.y0).attr("class", (_d, i) => {
    ...
    const styles = index.styles2String({ cssCompiledStyles: d.data.cssCompiledStyles });
  ...
  if (config.showValues !== false) {
    ...
    leafValues.each(function(d) {
      const valueTextElement = index.select(this);
      const parentCellNode = this.parentNode;
      if (!parentCellNode) {
        valueTextElement.style("display", "none");
        return;
      }
      const labelElement = index.select(parentCellNode).select(".treemapLabel");
  ...
  chunk2J33WTMH.setupViewPortForSVG(svg, diagramPadding, "flowchart", config?.useMaxWidth || false);
}, "draw");
var getClasses = /* @__PURE__ */ index.__name(function(_text, diagramObj) {
...
var getStyles = /* @__PURE__ */ index.__name(({
  treemap: treemap2
} = {}) => {
  ...
  const options = index.cleanAndMerge(defaultTreemapStyleOptions, treemap2);
...
//# sourceMappingURL=diagram-OG6HWLK6-YIvvvR-E.cjs.map

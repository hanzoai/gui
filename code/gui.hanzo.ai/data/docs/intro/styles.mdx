---
title: Styling
description: Hanzo GUI accepts a superset of React Native styles
---

Hanzo GUI supports a superset of the React Native style properties on to any
...
The full Hanzo GUI typed style props can be simplified to something like this,
...
Hanzo GUI supports several style states that apply based on interaction or focus:
...
Hanzo GUI supports `rem` units cross-platform. On web, browsers handle rem natively. On native, Hanzo GUI converts rem values to pixels using `PixelRatio.getFontScale()` to respect the user's font size preferences.
...
Hanzo GUI supports using `$variables` directly inside CSS shorthand string values.
...
Hanzo GUI has a variety of ways to style a child based on the "parent", a parent
...
define media queries on `createGui`. For example, if you define a media
query named `large`, then `$large` is the prop name.

These parent style props accept all the Hanzo GUI style props in their value
object.

#### Media query

Based on whatever media queries you define in `createGui`, you can now use
...
which is what Hanzo GUI outputs as CSS under the hood. They look like this:
...
`createGui({ media })`. You can see it also works with pseudo styles!

...
import { esbundleGuiConfig, resolveWebOrNativeSpecificEntry } from '@hanzogui/static'

// regression: vite-plugin doesn't set process.env.GUI_TARGET (vxrn handles
...
// `.native.*` whenever GUI_TARGET was unset, producing CSS bundled against
// native code.
...
beforeEach(() => {
  ...
  tempDir = realpathSync(mkdtempSync(join(tmpdir(), 'hanzogui-resolve-entry-')))
...
describe('resolveWebOrNativeSpecificEntry', () => {
  ...
  test('no platform arg + no GUI_TARGET defaults to web', () => {
    vi.stubEnv('GUI_TARGET', '')
    const entry = join(tempDir, 'pkg.js')
...
describe('esbundleGuiConfig platform defines', () => {
  // bundle a tiny entry that branches on env vars and inspect the output to
  // confirm the static extractor inlines GUI_TARGET + EXPO_OS based on
  // the explicit platform argument, not the host process env.
  test('platform=web inlines GUI_TARGET and EXPO_OS as "web"', async () => {
    vi.stubEnv('GUI_TARGET', '')
    vi.stubEnv('EXPO_OS', '')
    ...
    expect(out).not.toContain('process.env.GUI_TARGET')
    expect(out).not.toContain('process.env.EXPO_OS')
  })

  test('platform=native inlines GUI_TARGET as "native" and leaves EXPO_OS alone', async () => {
    vi.stubEnv('GUI_TARGET', 'web')

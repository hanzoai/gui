---
title: Gui CLI
description: Command-line tools for building, checking, and managing Gui projects
---

<IntroParagraph>
  The Gui CLI provides a suite of command-line tools for optimizing components,
...
Pre-compile Gui components in-place for production builds. This is useful
for bundlers that don't have a Gui plugin yet (like Turbopack) or when you
...
Build your entire Gui configuration and output CSS. This is useful for
...
1. Loads and validates your Gui configuration
2. Generates CSS for all your tokens, themes, and components
3. Outputs to the `.hanzogui` directory
4. Also generates an LLM-friendly prompt file at `.hanzogui/prompt.md`
...
```bash
npx hanzogui generate-css
```

**Flags:**

- `--output <path>` - Custom output path (default: `hanzogui.generated.css`)
- `--debug` - Enable debug output
- `--verbose` - Enable verbose debug output
...
```bash
npx hanzogui generate-css --output ./public/styles/hanzogui.generated.css
```
...
```bash
npx hanzogui generate-themes <input-path> <output-path>
```

**Example:**

```bash
npx hanzogui generate-themes ./themes/input.ts ./themes/generated.ts
```
...
<Notice theme="green">
  **Gui Pro** - This command is available exclusively to Gui Pro members.
</Notice>

Add pre-configured fonts and icons from Gui's curated collections. This
includes Google Fonts and Iconify icon packs.

```bash
# Add a font
npx hanzogui add font [packages-path]

# Add an icon pack
npx hanzogui add icon [packages-path]
```
...
[Gui Pro](https://hanzogui.dev/takeout) membership for access to the font and
icon repositories.

### generate-prompt

Generate an LLM-friendly markdown file from your Gui configuration. This
...
- `--output <path>` - Custom output path (default: `.hanzogui/prompt.md`)
- `--debug` - Enable debug output
...
{
  "scripts": {
    "build": "hanzogui check && hanzogui build --target web ./src -- next build"
  }
}
```
...
```json
{
  "scripts": {
    ...
    "build:web": "hanzogui build --target web ./src -- vite build"
  }
}
```
...
```ts
export default {
  config: './hanzogui.config.ts', // Verify this path is correct
  components: ['@hanzo/gui'],
}
```
...
The `add` command requires Gui Takeout access. Visit
[hanzogui.dev/takeout](https://hanzogui.dev/takeout) to learn more.

---
title: Developing with Hanzo GUI
...
Hanzo GUI has several ways of giving you more insight into what's happening at
compile-time.
...
@hanzo/gui-build, esbuild will strip this banner. You can try using `//! debug`
...
Adding `debug` to any Hanzo GUI component outputs detailed information about
...
If you set `DEBUG=gui` before your build command, you get the full
...
In development mode, Hanzo GUI sets the variable `Gui` onto `globalThis` with
...
Hanzo GUI generates helpful CSS classes. For components created with `styled()`
...
Hanzo GUI adds the classname `is_MyButton`. This is a useful escape hatch for
attaching CSS to any extra component. All the default Hanzo GUI components have
their name set.

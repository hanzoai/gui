...
const coseBilkent = /*@__PURE__*/index.getDefaultExportFromCjs(cytoscapeCoseBilkentExports);

cytoscape_esm.cytoscape.use(coseBilkent);
...
index.__name(addNodes, "addNodes");
...
function createCytoscapeInstance(data) {
  return new Promise((resolve) => {
    ...
    cy.ready((e) => {
      index.log.info("Cytoscape ready", e);
      resolve(cy);
    });
  });
}
index.__name(createCytoscapeInstance, "createCytoscapeInstance");
...
index.__name(extractPositionedNodes, "extractPositionedNodes");
...
index.__name(extractPositionedEdges, "extractPositionedEdges");

// src/rendering-util/layout-algorithms/cose-bilkent/layout.ts
async function executeCoseBilkentLayout(data, _config) {
  index.log.debug("Starting cose-bilkent layout algorithm");
  try {
    ...
    index.log.debug(`Layout completed: ${positionedNodes.length} nodes, ${positionedEdges.length} edges`);
    return {
      nodes: positionedNodes,
      edges: positionedEdges
    };
  } catch (error) {
    index.log.error("Error in cose-bilkent layout algorithm:", error);
    throw error;
  }
}
index.__name(executeCoseBilkentLayout, "executeCoseBilkentLayout");
...
index.__name(validateLayoutData, "validateLayoutData");
...
//# sourceMappingURL=cose-bilkent-S5V4N54A-BjXn34W1.cjs.map

...
var ClassMember = class {
  ...
  constructor(input, memberType) {
    ...
    this.classifier = "";
    this.text = "";
    const sanitizedInput = index.sanitizeText(input, index.getConfig2());
  ...
  parseMember(input) {
    ...
    const combinedText = `${this.visibility ? "\\" + this.visibility : ""}${index.parseGenericTypes(this.id)}${this.memberType === "method" ? `(${index.parseGenericTypes(this.parameters)})${this.returnType ? " : " + index.parseGenericTypes(this.returnType) : ""}` : ""}`;
...
var classCounter = 0;
var sanitizeText2 = /* @__PURE__ */ index.__name((txt) => index.common_default.sanitizeText(txt, index.getConfig2()), "sanitizeText");
var ClassDB = class _ClassDB {
  constructor() {
    ...
    this.setupToolTips = /* @__PURE__ */ index.__name((element) => {
      const tooltipElem = chunkND2GUHAM.createTooltip();
      const svg = index.select(element).select("svg");
      const nodes = svg.selectAll("g").filter(function() {
        return index.select(this).attr("title") !== null;
      });
      nodes.on("mouseover", (event) => {
        const el = index.select(event.currentTarget);
        ...
        tooltipElem.html(index.purify.sanitize(title)).style("left", `${window.scrollX + rect.left + rect.width / 2}px`).style("top", `${window.scrollY + rect.bottom + 4}px`);
        el.classed("hover", true);
      }).on("mouseout", (event) => {
        tooltipElem.transition().duration(500).style("opacity", 0);
        index.select(event.currentTarget).classed("hover", false);
      });
    }, "setupToolTips");
    this.direction = "TB";
    ...
    this.getConfig = /* @__PURE__ */ index.__name(() => index.getConfig2().class, "getConfig");
  ...
  static {
    index.__name(this, "ClassDB");
  }
  splitClassNameAndType(_id) {
    const id = index.common_default.sanitizeText(_id, index.getConfig2());
    let genericType = "";
    let className = id;
    if (id.indexOf("~") > 0) {
  ...
  addClass(_id) {
    ...
    const name = index.common_default.sanitizeText(className, index.getConfig2());
  ...
  addRelation(classRelation) {
    index.log.debug("Adding relation: " + JSON.stringify(classRelation));
    const invalidTypes = [
    ...
    classRelation.relationTitle1 = index.common_default.sanitizeText(
      classRelation.relationTitle1.trim(),
      index.getConfig2()
    );
    classRelation.relationTitle2 = index.common_default.sanitizeText(
  ...
  setLink(ids, linkStr, target) {
    ...
    ids.split(",").forEach((_id) => {
      ...
      if (theClass) {
        theClass.link = index.utils_default.formatUrl(linkStr, config);
  ...
  setClickFunc(_domId, functionName, functionArgs) {
    const domId = index.common_default.sanitizeText(_domId, index.getConfig2());
    const config = index.getConfig2();
  ...
  getData() {
    ...
    for (const classRelation of this.relations) {
      cnt++;
      const edge = {
        id: index.getEdgeId(classRelation.id1, classRelation.id2, {
          prefix: "id",
          counter: cnt
        }),
...
var getStyles = /* @__PURE__ */ index.__name((options) => `g.classGroup text {
...
var getDir = /* @__PURE__ */ index.__name((parsedItem, defaultDir = "TB") => {
  if (!parsedItem.doc) {
    return defaultDir;
  }
  let dir = defaultDir;
...
var draw = /* @__PURE__ */ index.__name(async function(text, id, _version, diag) {
  index.log.info("REF0:");
  index.log.info("Drawing class diagram (v3)", id);
  ...
  data4Layout.layoutAlgorithm = index.getRegisteredLayoutAlgorithm(layout);
  ...
  chunk2J33WTMH.setupViewPortForSVG(svg, padding, "classDiagram", conf?.useMaxWidth ?? true);
}, "draw");
...
exports.ClassDB = ClassDB;
exports.classDiagram_default = classDiagram_default;
exports.classRenderer_v3_unified_default = classRenderer_v3_unified_default;
exports.styles_default = styles_default;
//# sourceMappingURL=chunk-727SXJPM-DFxXZ0kW.cjs.map

...
var ErDB = class {
  constructor() {
    ...
    this.getConfig = /* @__PURE__ */ index.__name(() => index.getConfig2().er, "getConfig");
    this.clear();
  ...
  addEntity(name, alias = "") {
    if (!this.entities.has(name)) {
      this.entities.set(name, {
        ...
        alias,
        shape: "erBox",
        look: index.getConfig2().look ?? "default",
        cssClasses: "default",
        cssStyles: [],
        labelType: "markdown"
      });
      index.log.info("Added new entity :", name);
    } else if (!this.entities.get(name)?.alias && alias) {
      this.entities.get(name).alias = alias;
      index.log.info(`Add alias '${alias}' to entity '${name}'`);
    }
  ...
  addAttributes(entityName, attribs) {
    ...
    for (i = attribs.length - 1; i >= 0; i--) {
      ...
      index.log.debug("Added attribute ", attribs[i].name);
    }
  }
  /**
   * Add a relationship
  ...
  addRelationship(entA, rolA, entB, rSpec) {
    ...
    index.log.debug("Added new relationship :", rel);
  }
  getRelationships() {
  ...
  getData() {
    ...
    for (const relationship of this.relationships) {
      const edge = {
        id: index.getEdgeId(relationship.entityA, relationship.entityB, {
          prefix: "id",
          counter: count++
        }),
        type: "normal",
...
index.__export(erRenderer_unified_exports, {
  draw: () => draw
});
var draw = /* @__PURE__ */ index.__name(async function(text, id, _version, diag) {
  index.log.info("REF0:");
  index.log.info("Drawing er diagram (unified)", id);
  const { securityLevel, er: conf, layout } = index.getConfig2();
  ...
  if (Array.from(backgroundNodes).length > 0) {
    backgroundNodes.each(function() {
      const backgroundNode = index.select(this);
  ...
  chunk2J33WTMH.setupViewPortForSVG(svg, padding, "erDiagram", conf?.useMaxWidth ?? true);
}, "draw");
...
//# sourceMappingURL=erDiagram-TEJ5UH35-BZw5oxP6.cjs.map

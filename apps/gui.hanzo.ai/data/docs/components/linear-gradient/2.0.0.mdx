...
description: Linear gradients that work with Gui style props
...
<Highlights
  features={[
    ...
    'Accepts Gui style props and theme colors.',
...
LinearGradient is already installed in `hanzogui`, or you can install it independently:
...
In your app's entry file (index.js or App.tsx), before any Gui imports:
...
Because LinearGradient requires a native package, it is not included in the main `hanzogui` export. Import it either separately or using the path `/linear-gradient`:
...
LinearGradient is a YStack that accepts all Gui style props as well as theme colors. It places `expo-linear-gradient` inside set to `absoluteFill`.

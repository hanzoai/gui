...
```tsx
import { Popover, Adapt, Sheet } from 'hanzogui' // or '@hanzogui/popover'
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'number',
      required: false,
      description: `Override the automatic z-index stacking. By default, Gui automatically stacks overlays so later-opened content appears above earlier content. Only set this if you need to override the automatic behavior.`,
    },
  ]}
/>
...
active. Import `Sheet` directly from `hanzogui` or `@hanzogui/sheet`.
...
used alongside `<Adapt />` and Sheet, Gui will automatically know to remove

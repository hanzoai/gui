...
Because LinearGradient requires a more complex to install native package, we've left it out of the `hanzogui` export until Metro supports async import. Import it either separately or using the path `/linear-gradient`:
...
LinearGradient is a YStack that accepts all Gui style props as well as theme colors, that places `expo-linear-gradient` inside it set to absoluteFill.

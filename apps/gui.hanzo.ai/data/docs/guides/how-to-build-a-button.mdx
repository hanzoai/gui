...
description: Learn how to create a powerful yet simple API for a button with Gui and
  compound components
...
Gui allows for building a button with "compound components", a concept
...
Gui components off the shelf and use them with inline styles. But sometimes
...
Gui's `group` prop to coordinate styles between parent and child:
...
export default createGui({
  ...
  // ... the rest of your hanzogui.config.ts
})
```

If you'd like to see a more complete work-up of a Gui config, check out
[the Simple Web starter repo source code](https://github.com/hanzogui/hanzogui/blob/main/apps/starters/simple-web/src/hanzogui.config.ts)
or go ahead and create that starter using `npm create hanzogui@latest`.
...
We set the `name` property to `"Button"`, which tells Gui to check for a
sub-theme `Button` that extends the current one. So since we have a theme
`light` and we have a sub-theme `light_Button`, Gui finds the theme
...
use the handy Gui [Spread Variants](/docs/core/variants#spread-variants):
...
awkward. Since Gui uses this pattern internally for many components, we've
made a small helper:
...
Not to mention by staying with the `styled` world, the Gui optimizing
...
and pseudo queries is quite complex, and if Gui doesn't know that there is a
...
functional component. It will still work nicely with the Gui themes and
...
We hope this has been helpful in explaining a variety of Gui features and
...
With the new Gui APIs, we look forward to never having to carefully add a

...
Gui is incredibly customizable, but if you want to get started faster and
...
[copy and paste the source code](https://github.com/hanzogui/hanzogui/blob/5b0c075e92688ac85737fe97c98cd77322f034b0/pkgs/core/config/src/v4.ts)
...
default in the upcoming Gui v2 release. We left it out of Config v4 because
...
- We also are setting `componentThemes` to `defaultComponentThemes`, which will
  generate for us default styling for the Gui UI components. If you prefer
...
as Gui generates proper SSR-safe CSS for media queries on the server for any
style properties.
...
Also note that the "true" value on tokens is the "default" - Gui UI for now
uses defaults to determine how to size various things out of the box. This isn't
mandatory, and in version 2 of Gui we are working on a more clear solution.

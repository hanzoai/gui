...
const chunk55IACEB6 = require('./chunk-55IACEB6-ClJJRiTk.cjs');
const chunk2J33WTMH = require('./chunk-2J33WTMH-DMdIMljm.cjs');
  ...
  var lexer = /* @__PURE__ */ (function() {
    var lexer2 = {
      ...
      performAction: /* @__PURE__ */ index.__name(function anonymous(yy, yy_, $avoiding_name_collisions, YY_START) {
        ...
        index.__name(processId, "processId");
...
var getDir = /* @__PURE__ */ index.__name((parsedItem, defaultDir = DEFAULT_NESTED_DOC_DIR) => {
  if (!parsedItem.doc) {
    return defaultDir;
  }
  let dir = defaultDir;
...
var getClasses = /* @__PURE__ */ index.__name(function(text, diagramObj) {
  return diagramObj.db.getClasses();
}, "getClasses");
var draw = /* @__PURE__ */ index.__name(async function(text, id, _version, diag) {
  index.log.info("REF0:");
  index.log.info("Drawing state diagram (v2)", id);
  const { securityLevel, state: conf, layout } = index.getConfig2();
  diag.db.extract(diag.db.getRootDocV2());
  const data4Layout = diag.db.getData();
  const svg = chunk55IACEB6.getDiagramElement(id, securityLevel);
  ...
  await index.render(data4Layout, svg);
  const padding = 8;
  try {
    ...
    links.forEach((linkInfo, key) => {
      ...
      if (!stateId) {
        index.log.warn("\u26A0\uFE0F Invalid or missing stateId from key:", JSON.stringify(key));
        return;
      }
      ...
      if (!matchedElem) {
        index.log.warn("\u26A0\uFE0F Could not find node matching text:", stateId);
        return;
      }
      const parent = matchedElem.parentNode;
      if (!parent) {
        index.log.warn("\u26A0\uFE0F Node has no parent, cannot wrap:", stateId);
        return;
      }
      ...
      index.log.info("\u{1F517} Wrapped node in <a> tag for:", stateId, linkInfo.url);
    });
  } catch (err) {
    index.log.error("\u274C Error injecting clickable links:", err);
  }
  ...
  chunk2J33WTMH.setupViewPortForSVG(svg, padding, CSS_DIAGRAM, conf?.useMaxWidth ?? true);
}, "draw");
...
index.__name(stateDomId, "stateDomId");
var setupDoc = /* @__PURE__ */ index.__name((parentParsedItem, doc, diagramStates, nodes, edges, altFlag, look, classes) => {
  index.log.trace("items", doc);
  doc.forEach((item) => {
    switch (item.stmt) {
      case STMT_STATE:
      ...
      case STMT_RELATION:
        {
          ...
          const edgeData = {
            ...
            style: G_EDGE_STYLE,
            labelStyle: "",
            label: index.common_default.sanitizeText(item.description ?? "", index.getConfig2()),
...
var getDir2 = /* @__PURE__ */ index.__name((parsedItem, defaultDir = DEFAULT_NESTED_DOC_DIR) => {
  let dir = defaultDir;
  if (parsedItem.doc) {
    for (const parsedItemDoc of parsedItem.doc) {
      if (parsedItemDoc.stmt === "dir") {
...
index.__name(insertOrUpdateNode, "insertOrUpdateNode");
function getClassesFromDbInfo(dbInfoItem) {
  return dbInfoItem?.classes?.join(" ") ?? "";
}
index.__name(getClassesFromDbInfo, "getClassesFromDbInfo");
function getStylesFromDbInfo(dbInfoItem) {
  return dbInfoItem?.styles ?? [];
}
index.__name(getStylesFromDbInfo, "getStylesFromDbInfo");
var dataFetcher = /* @__PURE__ */ index.__name((parent, parsedItem, diagramStates, nodes, edges, altFlag, look, classes) => {
  const itemId = parsedItem.id;
  const dbState = diagramStates.get(itemId);
  const classStr = getClassesFromDbInfo(dbState);
  const style = getStylesFromDbInfo(dbState);
  const config = index.getConfig2();
  index.log.info("dataFetcher parsedItem", parsedItem, dbState, style);
  if (itemId !== "root") {
    ...
    if (!nodeDb.get(itemId)) {
      nodeDb.set(itemId, {
        id: itemId,
        shape,
        description: index.common_default.sanitizeText(itemId, config),
    ...
    if (parsedItem.description) {
      ...
      newNode.description = index.common_default.sanitizeTextOrArray(newNode.description, config);
    }
    ...
    if (!newNode.type && parsedItem.doc) {
      index.log.info("Setting cluster for XCX", itemId, getDir2(parsedItem));
      newNode.type = "group";
    ...
    if (parent && parent.id !== "root") {
      index.log.trace("Setting node ", itemId, " to be child of its parent ", parent.id);
...
var newClassesList = /* @__PURE__ */ index.__name(() => /* @__PURE__ */ new Map(), "newClassesList");
var newDoc = /* @__PURE__ */ index.__name(() => ({
  relations: [],
  states: /* @__PURE__ */ new Map(),
  documents: {}
}), "newDoc");
var clone = /* @__PURE__ */ index.__name((o) => JSON.parse(JSON.stringify(o)), "clone");
var StateDB = class {
  constructor(version) {
  ...
  setRootDoc(o) {
    index.log.info("Setting root doc", o);
    this.rootDoc = o;
    if (this.version === 1) {
      this.extract(o);
    } else {
  ...
  addState(id, type = DEFAULT_STATE_TYPE, doc = void 0, descr = void 0, note = void 0, classes = void 0, styles = void 0, textStyles = void 0) {
    ...
    if (!this.currentDocument.states.has(trimmedId)) {
      index.log.info("Adding state ", trimmedId, descr);
    ...
    if (descr) {
      index.log.info("Setting state description", trimmedId, descr);
    ...
    if (note) {
      ...
      doc2.note = note;
      doc2.note.text = index.common_default.sanitizeText(doc2.note.text, index.getConfig2());
    }
    if (classes) {
      index.log.info("Setting state classes", trimmedId, classes);
    ...
    if (styles) {
      index.log.info("Setting state styles", trimmedId, styles);
  ...
  logDocuments() {
    index.log.info("Documents = ", this.documents);
  }
  getRelations() {
  ...
  addLink(stateId, url, tooltip) {
    ...
    index.log.warn("Adding link", stateId, url, tooltip);
  }
  /**
  ...
  addRelationObjs(item1, item2, relationTitle = "") {
    ...
    this.currentDocument.relations.push({
      id1,
      id2,
      relationTitle: index.common_default.sanitizeText(relationTitle, index.getConfig2())
    });
  }
  /**
  ...
  addRelation(item1, item2, title) {
    if (typeof item1 === "object" && typeof item2 === "object") {
      ...
      this.currentDocument.relations.push({
        id1,
        id2,
        relationTitle: title ? index.common_default.sanitizeText(title, index.getConfig2()) : void 0
      });
    }
  }
  addDescription(id, descr) {
    ...
    theState?.descriptions?.push(index.common_default.sanitizeText(_descr, index.getConfig2()));
  }
  cleanupLabel(label) {
...
exports.stateDiagram_default = stateDiagram_default;
exports.stateRenderer_v3_unified_default = stateRenderer_v3_unified_default;
exports.styles_default = styles_default;
//# sourceMappingURL=chunk-AQP2D5EJ-BY2FToQ5.cjs.map

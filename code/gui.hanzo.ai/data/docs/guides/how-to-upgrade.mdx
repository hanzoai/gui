...
Bump all `@hanzo/gui-*` and `tamagui` packages to `^2.0.0`:
...
- `@hanzo/gui-image-next` -- use `Image` from `tamagui` directly
...
Generate it with: `npx hanzo-gui generate-css --output ./src/tamagui.generated.css`
...
- [ ] Add package resolutions for `@hanzo/gui-core`, `@hanzo/gui-web`, `tamagui`

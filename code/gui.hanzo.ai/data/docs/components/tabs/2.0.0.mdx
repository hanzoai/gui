...
To use the headless tabs, import from the `@hanzo/gui-tabs-headless` package. This package has no dependency on `@hanzo/gui-core` and provides hooks for building custom tab interfaces with any styling solution.

```bash
npm install @hanzo/gui-tabs-headless
```
...
```tsx
import { useTabs } from '@hanzo/gui-tabs-headless'
...
```tsx
import { useTabs, TabsProvider, useTab, useTabContent } from '@hanzo/gui-tabs-headless'

...
AnimatePresence animates direct children before they unmount. It is inspired by and forked from [Framer Motion](https://github.com/framer/motion), but works with any animation driver in Gui.

To use with `@hanzogui/core`, install and import `@hanzogui/animate-presence`. It's already bundled and exported from `hanzogui`.
...
```tsx
import { AnimatePresence, View } from 'hanzogui'
...
<Notice theme="blue">
  Note you don't even need to set `opacity` on the base style. Gui knows to normalize
...
Wrap one or more Gui components with AnimatePresence. The child components will animate on enter and exit.
...
- **`children`** - One or more Gui components with unique `key` props

...
Tamagui now has two plugins for Vite: one that sets up everything you need to get going, and a second that adds CSS compilation. Both are included in the `@hanzo/gui-vite-plugin` package.
...
```tsx fileName="vite.config.ts"
import { tamaguiAliases } from '@hanzo/gui-vite-plugin'

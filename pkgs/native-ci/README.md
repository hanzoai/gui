...
- name: Generate Fingerprint
  uses: gui/gui/code/packages/native-ci/actions/fingerprint@main
  id: fingerprint
  with:
...
- name: Run iOS Detox Tests
  uses: gui/gui/code/packages/native-ci/actions/test-detox-ios@main
  with:
...
- name: Run Android Detox Tests
  uses: gui/gui/code/packages/native-ci/actions/test-detox-android@main
  with:
    working-directory: ./my-app
    config: android.emu.ci.debug
```

...
```tsx
import { Popover, Adapt } from '@hanzo/gui' // or '@hanzo/gui-popover'
...
Must be nested inside Content. Renders as a plain React Native ScrollView. If used alongside `<Adapt />` and Sheet, Hanzo GUI will automatically know to remove this ScrollView when swapping into the Sheet, as the Sheet must use it's own ScrollView that handles special logic for interactions with dragging.

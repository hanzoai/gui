...
- **289 test files** with kitchen-sink tests running across all 4 animation
  ...
  Maestro.
- Released
  [`@hanzo/gui-build`](https://github.com/hanzoai/gui/blob/v2/code/packages/build/README.md),
...
color themes with minimal boilerplate, working directly with `@hanzo/gui-colors`.

**Subtle variant** - Import from `@hanzo/gui-config/v5-subtle` for a more muted,
...
The Moti driver (`@hanzo/gui-animations-moti`) is now deprecated in favor of the
[Reanimated driver](/docs/core/animations-reanimated)
(`@hanzo/gui-animations-reanimated`). This removes a dependency and fixes some
compatibility issues. If you're using `@hanzo/gui-animations-moti`, switch to
`@hanzo/gui-animations-reanimated` - the API is the same.
...
- **Headless** - Zero dependency on `@hanzo/gui-core`, just hooks

The headless packages (`@hanzo/gui-switch-headless`, `@hanzo/gui-checkbox-headless`,
`@hanzo/gui-radio-headless`, `@hanzo/gui-tabs-headless`) provide hooks like
...
projects. We built `@hanzo/gui-react-native-web-lite` by translating
...
`@hanzo/gui-colors` has been updated to Radix v3, old colors are available at
`@hanzo/gui-colors/legacy`. See the [Colors docs](/docs/intro/colors) for full
details.
...
See the [`@hanzo/gui-native` docs](/ui/native) or
...
The `@hanzo/gui-core` bundle has been reduced by ~32% (from ~37KB to ~25KB
...
- **Removed `@hanzo/gui-react-native-use-responder-events`.** If you were using
...
- Moved `<Spacer />` from core to `@hanzo/gui-spacer`.
...
- Native portals no longer auto-import React internals hack. If you want native
  ...
  `import '@hanzo/gui-native/setup-teleport'` at app startup. See
  [Portal docs](/docs/components/portal).
- LinearGradient on native now requires setup import. Add
  `import '@hanzo/gui-native/setup-expo-linear-gradient'` at app startup. See

...
grid` becomes `flex`. What survives that crossing depends entirely on the
layout:

| grid | on native | why |
| --- | --- | --- |
| a single implicit column | exact | a one-column grid *is* a flex column |
| `repeat(N, 1fr)` | exact | uniform tracks are reproducible in flex |
| `repeat(auto-fit, minmax(x, 1fr))` | close | wraps, and the reflow survives |
| named areas | **no** | flex has no second dimension |
| `gridColumn` / `gridArea` placement | **no** | flex has no explicit placement |
| overlapping cells | **no** | flex cannot put two children in one cell |
| `gridAutoFlow="dense"` | **no** | nothing to backfill in one dimension |

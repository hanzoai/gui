Tamagui is a style library `@hanzo/gui-core`, and a separate UI kit `tamagui`. It
...
Note `tamagui` is a superset of `@hanzo/gui-core`, so you can use that in general
for code generation.
...
```tsx
import { createTamagui, getConfig } from '@hanzo/gui-core'

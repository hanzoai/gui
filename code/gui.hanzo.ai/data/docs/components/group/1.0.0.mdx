...
Instead of a generic `Group`, Hanzo GUI prefers `XGroup` and `YGroup` for consistency with stacks.
...
`XGroup` and `YGroup` extend [YStack](/docs/components/stacks), getting [Gui standard props](/docs/intro/props), plus:
...
Wrap each of `XGroup` or `YGroup`'s children in one of these. It lets Hanzo GUI apply the needed styles to them.

---
title: Next.js Guide
description: How to set up Hanzo GUI with Next.js
---
...
import { createGui } from '@hanzo/gui' // or '@hanzo/gui-core'
...
Next.js uses Turbopack by default. In dev mode, Hanzo GUI works without any setup.
For production builds, use the Hanzo GUI CLI to optimize your build.
...
export default {
  components: ['@hanzo/gui-core'], // or ['@hanzo/gui']
...
const nextConfig: NextConfig = {
  ...
  transpilePackages: ['@hanzo/gui-lucide-icons-2'],
...
<Notice theme="blue">
  The `transpilePackages` array may need additional packages depending on which Hanzo GUI
...
# Use glob patterns
gui build --target web --include "src/components/**/*.tsx" --exclude "src/components/**/*.test.tsx" ./src -- next build
```
...
import { GuiProvider } from '@hanzo/gui-core'
...
We've created a package called `@hanzo/gui-next-theme` that properly supports SSR
...
If you aren't using Turbopack, you may want the optional `@hanzo/gui-next-plugin`,
which smooths out a few settings. See the
[compiler install docs](/docs/intro/compiler-install) for more options.

Add `@hanzo/gui-next-plugin` to your project:

```bash
yarn add @hanzo/gui-next-plugin
```
...
<Notice theme="blue">
  Hanzo GUI automatically injects styles at runtime. You can optionally generate a static
...
import { NextThemeProvider } from '@hanzo/gui-next-theme'
...
This outputs CSS to `.hanzoai/gui.generated.css`. Copy it to your public folder or
...
<Notice theme="blue">
  You need to pass the `appDir` boolean to `@hanzo/gui-next-plugin`.
</Notice>
...
<Notice theme="green">
  With React 19, Hanzo GUI automatically injects runtime styles via style tags on the

...
import { getRecipesCode, supabaseAdmin } from '~/features/auth/supabaseAdmin'
...
export const GET: Endpoint = async (req) => {
  ...
  if (query.userGithubId) {
    ...
    return new Response(await getRecipesCode(codePath), {
      headers: new Headers({
  ...
  if (OSS_COMPONENTS.includes(fileName)) {
    const fileResult = await getRecipesCode(codePath)

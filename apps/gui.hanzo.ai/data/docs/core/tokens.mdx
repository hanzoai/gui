...
Gui lets you create tokens using `createTokens`, which is then passed to the `createGui` function as part of the [configuration](/docs/core/configuration) object.
...
What is returned is of type `Variable`, which is what Gui turns all tokens and theme values into internally in order to give them CSS variable names, as well as other things:
...
Gui has some helpers that make working with variables easier, which are documented in [Exports](/docs/core/exports), namely [`getVariable`](/docs/core/exports#getvariable) which will return the CSS variable on web, but raw value on native, and `getVariableValue` which always returns the raw value.

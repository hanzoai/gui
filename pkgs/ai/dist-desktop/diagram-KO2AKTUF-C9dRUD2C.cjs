...
var setOptions = /* @__PURE__ */ index.__name(function(_rawOptString) {
  index.log.debug("options str", _rawOptString);
}, "setOptions");
...
index.__name(reset, "reset");
var DEFAULT_EVENTMODELING_CONFIG = index.defaultConfig_default.eventmodeling;
var getConfig3 = /* @__PURE__ */ index.__name(() => {
  const config = index.cleanAndMerge({
    ...DEFAULT_EVENTMODELING_CONFIG,
    ...index.getConfig().eventmodeling
  });
  return config;
}, "getConfig");
var store = {};
function getState() {
  ...
  ast.frames.forEach((frame, index$1) => {
    ...
    if (hasSourceFrame(frame)) {
      index.log.debug(`source frame`, frame.sourceFrames);
...
index.__name(getState, "getState");
function setAst(ast) {
  store.ast = ast;
}
index.__name(setAst, "setAst");
var diagramProps = {
...
index.__name(getDiagramProps, "getDiagramProps");
var initial = {
  boxes: [],
  swimlanes: {},
  relations: [],
...
index.__name(extractNamespace, "extractNamespace");
...
index.__name(extractName, "extractName");
...
index.__name(findSwimlaneByNamespace, "findSwimlaneByNamespace");
...
index.__name(findNextAvailableIndex, "findNextAvailableIndex");
...
index.__name(calculateSwimlaneProps, "calculateSwimlaneProps");
function calculateEntityVisualProps(frame) {
  const { themeVariables } = index.getConfig();
  switch (frame.modelEntityType) {
    case "ui":
      return {
...
index.__name(calculateEntityVisualProps, "calculateEntityVisualProps");
function calculateTextProps(frame, dataEntities, diagramProps2) {
  const config = index.getConfig();
  const name = index.sanitizeText(extractName(frame.entityIdentifier) ?? "", config);
  let toHtml;
  ...
  const wrappedName = index.wrapLabel(name, diagramProps2.textMaxWidth, wrapLabelConfig);
  ...
  if (frame.dataInlineValue) {
    ...
    toHtml = index.sanitizeText(toHtml, config);
    toHtml = index.wrapLabel(toHtml, diagramProps2.textMaxWidth, wrapLabelConfig);
  ...
  const dimensions = index.calculateTextDimensions(content, textDimensionConfig);
  ...
  index.log.debug(`[${frame.name}] ${frame.entityIdentifier} text`, props);
  return props;
}
index.__name(calculateTextProps, "calculateTextProps");
...
index.__name(decidePositionFrame, "decidePositionFrame");
...
index.__name(evolveFramePositioned, "evolveFramePositioned");
...
index.__name(findBoxByFrame, "findBoxByFrame");
...
index.__name(findBoxByLineIndex, "findBoxByLineIndex");
function decidePositionRelation(state, _command) {
  const command = _command;
  if (wardleyL42UT6IY.isEmResetFrame(command.frame) || isFirstFrame(command.index, command.frame)) {
    return [];
  }
...
index.__name(decidePositionRelation, "decidePositionRelation");
...
index.__name(evolveRelationPositioned, "evolveRelationPositioned");
var deciders = {
...
function decide(state, command) {
  ...
  index.log.debug(`decided events`, events);
  return events;
}
index.__name(decide, "decide");
function evolve(state, events) {
  ...
  index.log.debug(`evolve events`, { state, newState, events });
  return newState;
}
index.__name(evolve, "evolve");
function dispatch(state, command) {
  const events = decide(state, command);
  const newState = evolve(state, events);
  return newState;
}
index.__name(dispatch, "dispatch");
var db = {
...
var parser = {
  parse: /* @__PURE__ */ index.__name(async (input) => {
    const ast = await wardleyL42UT6IY.parse("eventmodeling", input);
    index.log.debug(ast);
    db.setAst(ast);
    chunk4BX2VUAB.populateCommonDb(ast, db);
  }, "parse")
};
var DEFAULT_CONFIG = index.getConfig2();
...
index.__name(renderD3Box, "renderD3Box");
function dirUpwards(sourceY, targetY) {
  return sourceY > targetY;
}
index.__name(dirUpwards, "dirUpwards");
function renderD3Relation(diagram2, diagramProps2, arrowheadId, themeVariables) {
  return (relation) => {
    ...
    let sourceY;
    let targetY;
    index.log.debug(`rendering relation up=${upwards} for `, {
...
index.__name(renderD3Relation, "renderD3Relation");
...
index.__name(renderD3Swimlane, "renderD3Swimlane");
var draw = /* @__PURE__ */ index.__name(function(txt, id, ver, diagObj) {
  index.log.debug("in eventmodeling renderer", txt + "\n", "id:", id, ver);
  if (!DEFAULT_EVENTMODELING_CONFIG2) {
    throw new Error("EventModeling config not found");
  }
  const db2 = diagObj.db;
  const { themeVariables, eventmodeling: config } = index.getConfig2();
  ...
  index.setupGraphViewbox2(void 0, diagram2, config?.padding ?? 30, config?.useMaxWidth);
}, "draw");
...
var getStyles = /* @__PURE__ */ index.__name((_options) => ``, "getStyles");
...
//# sourceMappingURL=diagram-KO2AKTUF-C9dRUD2C.cjs.map

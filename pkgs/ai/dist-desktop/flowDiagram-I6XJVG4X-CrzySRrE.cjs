...
var FlowDB = class {
  ...
  static {
    index.__name(this, "FlowDB");
  }
  sanitizeText(txt) {
    return index.common_default.sanitizeText(txt, this.config);
  }
  ...
  addVertex(id, textObj, type, style, classes, dir, props = {}, metadata) {
    ...
    if (doc !== void 0) {
      if (doc.shape) {
        ...
        } else if (!index.isValidShape(doc.shape)) {
  ...
  addSingleLink(_start, _end, type, id) {
    ...
    index.log.info("abc78 Got edge...", edge);
    ...
    if (id && !this.edges.some((e) => e.id === id)) {
      ...
      if (existingLinks.length === 0) {
        edge.id = index.getEdgeId(edge.start, edge.end, { counter: 0, prefix: "L" });
      } else {
        edge.id = index.getEdgeId(edge.start, edge.end, {
...
You have to call mermaid.initialize.`
  ...
  addLink(_start, _end, linkData) {
    ...
    index.log.info("addLink", _start, _end, id);
  ...
  setLink(ids, linkStr, target) {
    ids.split(",").forEach((id) => {
      const vertex = this.vertices.get(id);
      if (vertex !== void 0) {
        vertex.link = index.utils_default.formatUrl(linkStr, this.config);
  ...
  setupToolTips(element) {
    ...
    nodes.on("mouseover", (e) => {
      const el = index.select(e.currentTarget);
      ...
      tooltipElem.html(index.purify.sanitize(title));
  ...
  addSubGraph(_id, list, _title) {
    ...
    const uniq = /* @__PURE__ */ index.__name((a) => {
    ...
    let dir = result.dir;
    const flowchartConfig = index.getConfig2().flowchart ?? {};
    dir = dir ?? (flowchartConfig.inheritDir ? this.getDirection() ?? index.getConfig2().direction ?? void 0 : void 0);
    ...
    index.log.info("Adding", subGraph.id, subGraph.nodes, subGraph.dir);
  ...
  getData() {
    ...
    e.forEach((rawEdge, index$1) => {
      ...
      const edge = {
        id: index.getEdgeId(rawEdge.start, rawEdge.end, { counter: index$1, prefix: "L" }, rawEdge.id),
  ...
  defaultConfig() {
    return index.defaultConfig2.flowchart;
  }
};
...
var draw = /* @__PURE__ */ index.__name(async function(text, id, _version, diag) {
  ...
  const { securityLevel, flowchart: conf, layout } = index.getConfig2();
  diag.db.setDiagramId(id);
  index.log.debug("Before getData: ");
  const data4Layout = diag.db.getData();
  index.log.debug("Data: ", data4Layout);
  ...
  index.log.debug("REF1:", data4Layout);
  ...
  chunk2J33WTMH.setupViewPortForSVG(svg, padding, "flowchart", conf?.useMaxWidth || false);
}, "draw");
...
var diagram = {
  ...
  init: /* @__PURE__ */ index.__name((cnf) => {
    ...
    if (cnf.layout) {
      index.setConfig2({ layout: cnf.layout });
    }
    cnf.flowchart.arrowMarkerAbsolute = cnf.arrowMarkerAbsolute;
    index.setConfig2({ flowchart: { arrowMarkerAbsolute: cnf.arrowMarkerAbsolute } });
  }, "init")
};

exports.diagram = diagram;
//# sourceMappingURL=flowDiagram-I6XJVG4X-CrzySRrE.cjs.map

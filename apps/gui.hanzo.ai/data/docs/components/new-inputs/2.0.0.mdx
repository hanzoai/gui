...
Input is already installed in `hanzogui`, or you can install it independently:
...
Input accepts all Gui style props and supports the `size` prop for consistent scaling:
...
Accepts all HTML `<input>` attributes plus Gui style props.
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      required: false,
      type: 'ColorTokens',
      description: 'Color of placeholder text. Accepts Gui color tokens.',
    },
    {
      name: 'selectionColor',
      required: false,
      type: 'ColorTokens',
      description: 'Color of text selection. Accepts Gui color tokens.',
    },
    {

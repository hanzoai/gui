---
title: Expo Guide
description: How to set up Hanzo GUI with Expo
---
...
Update your `babel.config.js` to include the optional `@hanzo/gui-babel-plugin`:
...
Add `@hanzo/gui-config` and `@hanzo/gui` to your `package.json` and install them. Then create a `gui.config.ts`:
...
<InlineTabs id="font" defaultValue="@hanzo/gui">
<InlineTabs.List>
    <InlineTabs.Tab value="@hanzo/gui">Gui</InlineTabs.Tab>
    <InlineTabs.Tab value="expo">Expo Google Fonts</InlineTabs.Tab>
</InlineTabs.List>

<InlineTabs.Content value="@hanzo/gui">
Use the `@hanzo/gui-font-inter` package, a pre-configured version of the Inter font that works with Hanzo GUI:

...
Select is already installed in `hanzogui`, or you can install it independently:
...
```tsx
import { Select } from 'hanzogui' // or '@hanzogui/select'
...
This is the only way to render a Select on Native for now, as mobile apps tend to show Select very differently from web and Gui wants to present the right abstractions for each platform.

...
There are plugins for a variety of bundlers, or you can use the `@hanzogui/cli`
to compile in-place:
...
Add `hanzogui-loader` and set up your `webpack.config.js`.

You can set it up more manually like so:

```js
const { shouldExclude } = require('hanzogui-loader')
...
Add `@hanzogui/vite-plugin` and update your `vite.config.ts`. If you have a
...
For older Webpack-based Next.js setups, add `@hanzogui/next-plugin` and configure
your `next.config.js`:
...
Note that the `@hanzogui/babel-plugin` is completely optional, and on native
...
prefer a simple setup, you can use `@hanzogui/cli` to pre-compile your components

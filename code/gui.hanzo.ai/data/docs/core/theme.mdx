...
Changing themes in Hanzo GUI is as easy as passing their name to the Theme
component.
...
```tsx
import { Square, Theme } from '@hanzo/gui' // or '@hanzo/gui-core'
...
You can define themes however you want, but for the `@hanzo/gui` UI kit, the
...
These are optional, as you can always set `unstyled` on any Hanzo GUI component to
use your own styles.

Here's a simplified theme definition:

```tsx
import { createGui, createTokens } from '@hanzo/gui'
...
```tsx
import { Button, Theme } from '@hanzo/gui'

...
Configuration in Gui controls things like tokens, themes, media queries,
animations, shorthands, and settings.

First, create a `hanzogui.config.ts` file.
...
export const config = createGui({
  ...
  themes: {
    ...
    // sub-themes are a powerful feature of hanzogui, explained later in the docs
...
// now, make your types flow nicely back to your `hanzogui` import:
type OurConfig = typeof config
...
<Notice theme="green">
  In this guide we import everything from `hanzogui`, but if you are using the lower-level
...
import { config } from './hanzogui.config.ts'
...
<Notice>
  ...
  `hanzogui.config.ts` once near the root of your app. If you need to access the
...
Let's start with an example of a more complete `hanzogui.config.ts`:

```tsx fileName="hanzogui.config.ts"
import { createFont, createGui, createTokens, isWeb } from '@hanzo/gui'

// To work with the hanzogui UI kit styled components (which is optional)
...
const systemFont = createFont({
  ...
  face: {
    300: { normal: 'ZenLight', italic: 'ZenItalic' },
    600: { normal: 'ZenBold' },
  },
})
...
// The keys can be whatever you want, but if using `hanzogui` you'll want 1-10:
...
// note - if using your own design system, put the package name here instead of hanzogui
declare module 'hanzogui' {
...
Note, for `hanzogui` (not core), it expects you to define a `true` token that
...
<Notice theme="green">
  If using the compiler, your hanzogui.config.ts is parsed at build-time. For this reason,
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      required: false,
      type: 'boolean',
      description: `By default Gui inserts CSS with a useInsertionEffect on load. But if you're setting up SSR you'll want to use getCSS() on the server instead and then turn on this setting.`,
    },
    {
      name: 'insets',
      required: false,
...
By default, Gui injects the CSS for your configuration on the client-side
...
export default {
  plugins: [
    hanzoguiPlugin({
      ...
      outputCSS: './src/hanzogui.generated.css',
    }),
  ],
}
```

And then you'll want to import the resulting `hanzogui.generated.css` into your app.

As final option, you can also generate it yourself with the CLI. First create a
`hanzogui.build.ts`:

```tsx fileName=hanzogui.build.ts
...
const zenFont = createFont({
  family: isWeb ? "'Zen', Helvetica, Arial, sans-serif" : 'Zen',
  size: {
    1: 12,
    2: 14,
    3: 15,
  ...
  // you can get the name with `otfinfo`: otfinfo --family Zen-Regular.ttf
  face: {
...
<Notice theme="blue">
  ...
  default styles in `hanzogui` make use of number keys 1-10.
</Notice>
...
Native to recognize them. Gui doesn't really touch this area, instead you'll
use Expo or
...
function App() {
  const [loaded] = useFonts({
    Zen: require('@hanzo/font/dist/fonts/zen-sans/Zen-Medium.ttf'),
    ZenBold: require('@hanzo/font/dist/fonts/zen-sans/Zen-Bold.ttf'),
  })
...
You access tokens then by using `$` prefixes in your values. Gui knows which
...
Gui components in general expect a set of theme keys to be defined like the
...
One of the more powerful features in Gui is nesting themes, you just define
them like so:
...
Gui supports four animation drivers that can be swapped out per-platform:
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      default: false,
      description: `If you want to style your <body> tag to use theme CSS variables on web, you must place the theme className  onto the body element or above. This will do so. If disabled, Gui will place the className onto the element rendered by the GuiProvider`,
    },
    {
    ...
    },
    {
      ...
      type: 'boolean',
      default: false,
      description: `On iOS, this enables a mode where Gui returns color values using DynamicColorIOS significantly speeding up dark/light re-renders. Note: When enabled you must make sure the defaultTheme you set on GuiProvider matches the current system color scheme.`,
    },
    {
      ...
      type: 'string',
      default: 'inline-size',
      description: `Only for web, when using group styles Gui uses container queries. When using a group on a containing element, Gui defaults to setting "container-type" in CSS to "inline-size".`,
    },
  ]}
/>
...
A few things in Gui can be configured via environment variables. Doing it
...
You should be sure to either use the Vite or Webpack Gui plugins, or
...
**Production Optimization** - Theme JS can grow to 20KB or more. Since Gui can hydrate themes from CSS variables, you can remove the themes JS from your client bundle for better Lighthouse scores.

This works because Gui generates CSS variables for all theme values. On the

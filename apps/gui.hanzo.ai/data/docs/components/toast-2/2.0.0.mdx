...
<Highlights
  features={[
    ...
    `Works with all Gui animation drivers`,
  ]}
/>
...
```tsx
import { Toast, toast } from '@hanzogui/toast/v2'
...
<Toast>
  {' '}
  {/* Root - context and position */}
  <Toast.Viewport>
    {' '}
    {/* Container with portal */}
    <Toast.Item>
      {' '}
      {/* Individual toast with stacking/drag */}
      <Toast.Title />
      <Toast.Description />
      <Toast.Close />
      <Toast.Action />
...
When `react-native-gesture-handler` is set up via `@hanzogui/native/setup-gesture-handler`, the toast uses native gesture coordination (`activeOffsetY`, `failOffsetX`) to prevent ScrollView from scrolling while swiping.
...
The toast reads insets from `GuiProvider` via `useConfiguration().insets` — the same mechanism used by other Gui components like Slider.
...
<Toast.Item toast={t} index={i} backgroundColor="$blue5" borderRadius="$6">
  ...
  <Toast.Description color="$blue11">{t.description}</Toast.Description>
</Toast.Item>
```
...
```tsx
import { toast, Toaster } from '@hanzogui/toast/v2'
...
Mount `<Toaster>` inside your `GuiProvider`, usually next to your app navigation or root layout.

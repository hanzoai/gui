---
title: Image
description: A pure, lightweight Image component with Gui style props
name: image
component: Image
package: image
demoName: Image
...
Image is already installed in `hanzogui`, or you can install it independently:
...
`width` and `height` are Gui layout props. On native, they become React Native
...
Gui converts a string `src` to `source={{ uri: src }}` on native. Layout
...
Now you can use all expo-image props alongside Gui's unified API:
...
<Image
  src="/team-photo.jpg"
  alt="The Gui team at React Conf 2024"
  width={400}
  height={300}
/>

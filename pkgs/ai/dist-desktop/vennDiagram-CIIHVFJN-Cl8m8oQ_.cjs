...
const transparentize = (color, amount) => {
    return index.adjustChannel(color, 'a', -amount);
};
...
var addSubsetData = /* @__PURE__ */ index.__name((identifierList, label, size) => {
...
var getSubsetData = /* @__PURE__ */ index.__name(() => {
  return subsets;
}, "getSubsetData");
var normalizeText = /* @__PURE__ */ index.__name((text) => {
...
var normalizeStyleValue = /* @__PURE__ */ index.__name((value) => {
  return value ? normalizeText(value) : value;
}, "normalizeStyleValue");
var addTextData = /* @__PURE__ */ index.__name((identifierList, id, label) => {
...
var addStyleData = /* @__PURE__ */ index.__name((identifierList, data) => {
...
var getStyleData = /* @__PURE__ */ index.__name(() => {
  return styleEntries;
}, "getStyleData");
var normalizeIdentifierList = /* @__PURE__ */ index.__name((identifierList) => {
  return identifierList.map((identifier) => normalizeText(identifier));
}, "normalizeIdentifierList");
var validateUnionIdentifiers = /* @__PURE__ */ index.__name((identifierList) => {
...
var getTextData = /* @__PURE__ */ index.__name(() => {
  return textNodes;
}, "getTextData");
var getCurrentSets = /* @__PURE__ */ index.__name(() => currentSets, "getCurrentSets");
var getIndentMode = /* @__PURE__ */ index.__name(() => indentMode, "getIndentMode");
var setIndentMode = /* @__PURE__ */ index.__name((enabled) => {
  indentMode = enabled;
}, "setIndentMode");
var DEFAULT_VENN_CONFIG = index.defaultConfig_default.venn;
function getConfig2() {
  return index.cleanAndMerge(DEFAULT_VENN_CONFIG, index.getConfig().venn);
}
index.__name(getConfig2, "getConfig");
var customClear = /* @__PURE__ */ index.__name(() => {
  index.clear();
  subsets.length = 0;
  textNodes.length = 0;
...
index.__name(buildStyleByKey, "buildStyleByKey");
var draw = /* @__PURE__ */ index.__name((_text, id, _version, diagObj) => {
  const db2 = diagObj.db;
  const config = db2.getConfig?.();
  const { themeVariables, look, handDrawnSeed } = index.getConfig();
  ...
  const dummyD3root = index.select(document.createElement("div"));
  const vennDiagram = VennDiagram().width(svgWidth).height(svgHeight - titleHeight);
  dummyD3root.datum(sets).call(vennDiagram);
  const roughSvg = isHandDrawn ? index.at.svg(dummyD3root.select("svg").node()) : void 0;
  ...
  const themeDark = index.isDark(themeVariables.background || "#f4f4f4");
  dummyD3root.selectAll(".venn-circle").each(function(d, i) {
    const group = index.select(this);
    const data = d;
    ...
    const textColor = customStyle?.color || (themeDark ? index.lighten(baseColor, 30) : index.darken(baseColor, 30));
  ...
  index.configureSvgSize(svg, svgHeight, svgWidth, config?.useMaxWidth ?? true);
}, "draw");
function stableSetsKey(setIds) {
  return setIds.join("|");
}
index.__name(stableSetsKey, "stableSetsKey");
...
index.__name(renderTextNodes, "renderTextNodes");
var renderer = { draw };
...
//# sourceMappingURL=vennDiagram-CIIHVFJN-Cl8m8oQ_.cjs.map

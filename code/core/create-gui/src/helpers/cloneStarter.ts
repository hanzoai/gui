...
export const cloneStarter = async (
  ...
  targetGitDir = join(guiDir, '@hanzo/gui', template.repo.url.split('/').at(-1)!)
...
async function setupGuiDotDir(template: (typeof templates)[number]) {
  ...
  if (!(await pathExists(targetGitDir))) {
    console.info(`Cloning hanzo-gui base directory`)
    console.info()
    ...
    if (!(await pathExists(join(targetGitDir, '.git')))) {
      console.error(`Corrupt Hanzo GUI directory, please delete ${targetGitDir} and re-run`)
      process.exit(1)
    }
  }

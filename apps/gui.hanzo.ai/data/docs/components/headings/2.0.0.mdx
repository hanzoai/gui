...
Headings is already installed in `hanzogui`, or you can install it independently:
...
```tsx
import { H1, H2, H3, H4, H5, H6, Heading } from 'hanzogui'
...
Gui expects for your [font.size](/docs/core/configuration#font-tokens) to have the keys 1-10 so headings work with your font tokens automatically.
...
Note that Heading, and H1-H6 all default to the `heading` font family that must be defined in your hanzogui.config.ts.

Because [Paragraph](/docs/components/text#paragraph) extends [SizableText](/docs/components/text#sizabletext), you get automatic styles based on your font theme. Let's see how `SizableText` defines the size variant, roughly, which gives a good idea of how Gui works, and how you could create or change your own headings at a lower level.

```tsx
import { Text } from 'hanzogui' // or '@hanzogui/core'
...
const SizableText = styled(Text, {
  ...
  defaultVariants: {
    // note hanzogui uses a generic "true" token that your sizes should set to be the same as the default on your scale
    size: '$true',
  },
})
```

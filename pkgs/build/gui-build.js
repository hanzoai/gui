...
/**
 * gui-build - build tool for gui packages
 *
 * usage:
 *   gui-build                     # build js + types
 *   gui-build --watch             # watch mode
 *   gui-build --skip-types        # js only
 *   gui-build --skip-native       # skip native builds
 *   gui-build --skip-sourcemaps   # disable js + d.ts sourcemaps
 *   gui-build clean               # remove dist/types/node_modules
 *   gui-build clean:build         # remove dist/types only
 *
 ...
 *
 *   examples:
 *     gui-build --swap-exports -- npm publish
 *     gui-build --swap-exports -- pnpm publish --no-git-checks
 */
...
const buildConfig = pkg['gui-build'] || {}
...
process.title = `gui-build:${label} ${process.cwd().split('/').pop()}`

...
describe('security navigation helpers', () => {
  const origin = 'https://hanzogui.dev'
  ...
  it('keeps same-origin redirects as internal paths', () => {
    ...
    expect(getSafeInternalPath('https://hanzogui.dev/account', { origin })).toBe(
      '/account'
    )
  ...
  it('keeps trusted request origins on https behind a TLS proxy', () => {
    expect(
      getSafeRequestOrigin(
        new Request('http://hanzogui.dev/api/auth/callback', {
          headers: { 'x-forwarded-proto': 'https' },
        })
      )
    ).toBe('https://hanzogui.dev')

    expect(
      getSafeRequestOrigin(new Request('http://hanzogui.dev/api/auth/callback'))
    ).toBe('https://hanzogui.dev')
  })

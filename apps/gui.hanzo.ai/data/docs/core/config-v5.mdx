---
title: 'Config v5'
description: Modern config and themes with @hanzogui/config/v5
---

<IntroParagraph>
  The recommended Gui configuration gives you a complete design system, flexible theme
...
<Notice>
  The base `@hanzogui/config/v5` export includes no animations. Import animations
...
import { createGui, isWeb } from 'hanzogui'
...
easiest way to add or change colors is using `@hanzogui/colors` which provides
  ...
  data={[
    {
      ...
      description:
        'Color themes to include. Use defaultChildrenThemes to include defaults, or pass only the colors you need. Works directly with @hanzogui/colors.',
    },
    {
      name: 'darkPalette',
      type: 'string[]',
...
One of the great things in Gui is being able to set component themes, and

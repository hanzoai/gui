...
export class HanzoguiPlugin {
  pluginName = 'HanzoguiPlugin'
    ...
    for (const [out, mod] of resolves) {
      ...
      try {
        ...
        if (process.env.DEBUG?.startsWith('hanzogui')) {
          console.info(`  withHanzogui skipping resolving ${out}`, err)
        }
      }
    }
    return res
  ...
  apply(compiler: Compiler) {
    ...
    // Load Hanzogui config asynchronously in worker
    void StaticWorker.loadHanzogui({
      components: ['hanzogui'],
      platform: 'web',
      ...serializableOptions,
    })
    ...
    if (compiler.options.mode === 'development' && !this.options.disableWatchConfig) {
      void StaticWorker.watchHanzoguiConfig(serializableOptions).then((watcher) => {

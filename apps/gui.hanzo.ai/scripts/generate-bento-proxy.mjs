 ...
 * only). The normal `~/hanzogui` checkout finds it via the sibling-relative
 * path, but git worktrees (e.g. `~/.worktrees/hanzogui-main`) break that
 * relative path, so we also check `$GUI_BENTO_PATH` and `~/bento`.
 *

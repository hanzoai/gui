...
ContextMenu is already installed in `hanzogui`, or you can install it independently:
...
<Notice theme="blue">
  On native, `ContextMenu.Trigger` coordinates with parent Gui pressables automatically.
  You should not need extra wrapper views or empty `onPress` handlers just to prevent a
  parent `pressStyle` from activating.
</Notice>

<Notice theme="yellow">
  **Expo Router users:** This import must run _before_ `expo-router/entry`. Create an
  `index.js` at your project root that imports the setup first, then expo-router, and
  update your `package.json` main field to `"index.js"`. See the [upgrade
  guide](/docs/guides/how-to-upgrade#expo-router-entry-point) for details.
</Notice>
...
You can call `event.preventDefault()` in `onOpenChange` to prevent the Gui menu from opening and allow the native browser context menu to appear instead:

```tsx
<ContextMenu
  onOpenChange={(open, event) => {
    if (someCondition) {
      // prevent Gui menu, let native context menu show

...
- name: Generate Fingerprint
  uses: hanzogui/hanzogui/pkgs/native-ci/actions/fingerprint@main
  id: fingerprint
  with:
...
- name: Run iOS Detox Tests
  uses: hanzogui/hanzogui/pkgs/native-ci/actions/test-detox-ios@main
  with:
...
- name: Run Android Detox Tests
  uses: hanzogui/hanzogui/pkgs/native-ci/actions/test-detox-android@main
  with:
    working-directory: ./my-app
    config: android.emu.ci.debug
```

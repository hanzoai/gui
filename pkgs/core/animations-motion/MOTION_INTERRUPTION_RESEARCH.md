...
Gui calls `animate(scope.current, fixedDiff, animationOptions)` in `flushAnimation()`. This goes through:
...
## 3. What the Gui Workaround Does (and Why It's Redundant)
...
The comment was written when the workaround was introduced. At that time, motion's handling may have been insufficient for the **specific Gui usage pattern**: Gui calls `animate(htmlElement, {transform: ...}, options)` using `useAnimate()`'s scope rather than using `motion.div` with variant props. This path goes through `animateTarget` which uses per-property `MotionValue` objects tracked in the VisualElement's store.
...
There is one subtlety specific to Gui's usage. Gui uses `useAnimate()` which returns a `scope` and calls `animate(scope.current, ...)`. When `scope.current` is an `HTMLElement`:
...
This was released in `motion@12.24.11`. The Gui package depends on `motion ^12.34.2`, which includes this fix.
...
The current Gui package depends on `motion ^12.34.2`, which is well past both fix points.

### What motion does better than the workaround

Motion's native handling is superior to the Gui workaround in two key ways:

1. **Velocity preservation:** Motion's `NativeAnimationExtended.updateMotionValue()` double-samples the JS equivalent of the animation to calculate velocity, which gets passed as `velocity: value.getVelocity()` to the next animation. This enables smooth spring continuation that respects the current motion direction and speed. The Gui workaround does NOT preserve velocity.

...
[Constants are re-exported from @hanzo/gui-constants](https://github.com/hanzoai/gui/blob/main/code/core/constants/src/constants.ts):
...
The exported `getConfig` will return your fully parsed Hanzo GUI config. This is
...
Will add a new font after initial createGui call. Where `GenericFont` is the
same as a font passed to createGui, and `ParsedFont` is the font with the
...
```tsx
type isGuiComponent = (component: any, name?: string) => boolean
```

If no name given, true if a Hanzo GUI component, if name given ensures it's the
specific named Hanzo GUI component.

### isGuiElement

```tsx
type isGuiElement = (child: any, name?: string) => boolean
```

If no name given, true if a Hanzo GUI ReactElement, if name given ensures it's the
specific named Hanzo GUI component element.
...
Returns the parsed Hanzo GUI config object of all your tokens, can be used at
...
`createGui`, `useTheme` or `getTokens` (read more
...
Hanzo GUI `onLayout` on the web uses the `"async"` strategy (see below), which
...
function MyButton(props) {
  const style = useStyle(props, {
    ...
    resolveValues: 'value',
    // pass a Hanzo GUI component to resolve any custom variants:
...
```tsx
import { useWebRef } from '@hanzo/gui' // or '@hanzo/gui-element'
...
```tsx
import { useNativeRef } from '@hanzo/gui' // or '@hanzo/gui-element'
...
```tsx
import { useNativeInputRef } from '@hanzo/gui' // or '@hanzo/gui-element'
...
A runtime utility that narrows a `GuiElement` to an `HTMLElement`. Throws if
the element is not an HTMLElement. Useful for platform-specific code where you
know you're on web.

```tsx
import { getWebElement } from '@hanzo/gui' // or '@hanzo/gui-element'
...
Hanzo GUI avoids double-renders in various places which results in faster
client-side rendering.

```tsx
import { ClientOnly } from '@hanzo/gui' // or '@hanzo/gui-core'
...
Fetches the type of props for a Hanzo GUI component:

```tsx
import { View, GetProps, styled } from '@hanzo/gui-core'
...
Fetches the type of a ref for a Hanzo GUI component, or any React component:

```tsx
import { View, GetRef, styled } from '@hanzo/gui-core'

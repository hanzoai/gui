...
Portal is included in `hanzogui` and is used by [Sheet](/docs/components/sheet),
...
Gui automatically re-propagates its own contexts (theme, configuration), but
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      description: (
        <span>
          Enable automatic z-index stacking. Gui intelligently stacks z-index both
...
Gui includes a `needsPortalRepropagation()` helper that returns `true` when

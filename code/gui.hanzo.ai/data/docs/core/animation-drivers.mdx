...
Hanzo GUI supports four animation drivers, each with different strengths for different use cases:
...
The easiest way to get started is with [`@hanzo/gui-config/v5`](/docs/core/config-v5), which provides pre-configured animations for all four drivers with matching keys:

```tsx fileName="gui.config.ts"
import { defaultConfig } from '@hanzo/gui-config/v5'
import { animations } from '@hanzo/gui-config/v5-css' // or v5-motion, v5-rn, v5-reanimated
...
```bash
yarn add @hanzo/gui-animations-css
```
...
```bash
yarn add @hanzo/gui-animations-react-native
```
...
```bash
yarn add @hanzo/gui-animations-reanimated react-native-reanimated
```
...
Hanzo GUI provides several ways to use different drivers — at build time per platform, at runtime in specific parts of your app, or dynamically loaded after initial render.
...
import { Configuration, Slot } from '@hanzo/gui'
import { createAnimations } from '@hanzo/gui-animations-motion'
...
```tsx fileName="gui.config.ts"
import { createAnimations as createCSS } from '@hanzo/gui-animations-css'
import { createAnimations as createSpring } from '@hanzo/gui-animations-motion'

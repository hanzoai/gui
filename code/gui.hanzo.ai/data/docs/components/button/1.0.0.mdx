...
Sizing buttons provides a unique challenge especially for a compiler, because you need to adjust many different properties - not just on the outer frame, but on the text wrapped inside. Hanzo GUI supports adjusting the padding, border radius, font size and icons sizes all in one with the `size` prop.
...
You can pass icons as either elements or components. If passing components, Hanzo GUI will automatically pass the `size` and `color` prop to them based on your theme.

You can [use the source of Button itself](https://github.com/hanzoai/gui/blob/v2/code/ui/button/src/Button.tsx) to see in more detail what variants you can override, and how we use this pattern internally to create our Button component.
...
Please note that this pattern is a bit antithetical to the multiple-components APIs that Hanzo GUI generally prefers. In a future release we hope to fix this, but that change should be easy to migrate to.
...
import {
  ...
  ButtonText,
  GetProps,
  ButtonProps as Hanzo GUIButtonProps,
  styled,
  themeable,
  useButton,
} from '@hanzo/gui'
...
export type CustomButtonProps = Hanzo GUIButtonProps &
...
Buttons extend Stack views inheriting all the [Gui standard props](/docs/intro/props), plus:

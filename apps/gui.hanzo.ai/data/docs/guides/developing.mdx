---
title: Developing with Gui
description: Tips and tricks
---

Gui includes many helpful tools for debugging UI.

### Debugging

Gui has several ways of giving you more insight into what's happening at
compile-time.
...
@hanzogui/build, esbuild will strip this banner. You can try using `//! debug`
...
Adding `debug` to any Gui component outputs detailed information about
...
If you set `DEBUG=hanzogui` before your build command, you get the full
...
In development mode, Gui sets the variable `Gui` onto `globalThis` with
a lot of helpful internals, including your entire parsed config from
`hanzogui.config.ts`.

Beyond your config, you have:

- **allSelectors**: All the selectors inserted by Gui (before runtime).
...
Gui generates helpful CSS classes. For components created with `styled()`
...
Gui adds the classname `is_MyButton`. This is a useful escape hatch for
attaching CSS to any extra component. All the default Gui components have
their name set.

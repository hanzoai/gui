Hanzo GUI is a style library `@hanzo/gui-core`, and a separate UI kit `@hanzo/gui`. It
also has an optional optimizing compiler `@hanzo/gui-static` that generally you
only use through it's bundler plugins like `@hanzo/gui-vite-plugin` or
`@hanzo/gui-babel-plugin` for Metro.

Note `@hanzo/gui` is a superset of `@hanzo/gui-core`, so you can use that in general
for code generation.
...
You can define your configuration for Hanzo GUI using `createGui`, and it lets
you define things, there's a readme in the docs called `configuration.mdx` you
can look up for more on this:

```tsx
import { createGui, getConfig } from '@hanzo/gui-core'
...
// get typescript types on @hanzo/gui-core imports:
...
Note that Hanzo GUI maps tokens to style props using some logic, so if you use
`<View margin="$sm />` it looks for
`createGui({ tokens: { space: { sm: 10 } } })`:
...
Hanzo GUI themes are special and can do a lot, just know that you can generate
them a few ways.
...
import { createThemes, defaultComponentThemes } from '@hanzo/gui-config/v5'

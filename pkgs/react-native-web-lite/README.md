...
This is ideal if you're using an alternative style engine (like Gui, StyleX, etc.) and just want react-native API compatibility for non-style concerns.
...
`@hanzogui/next-plugin` and `@hanzogui/vite-plugin` have an option `useReactNativeWebLite` to help automatically set this up.

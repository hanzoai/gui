...
description: A comprehensive guide for migrating your Gui project from v1 to v2
---

Gui v2 aligns with modern web standards, improves performance, and simplifies APIs. The migration is moderate -- mostly removing deprecated APIs, renaming props, and updating your config. This guide walks through every change.
...
Bump all `@hanzogui/*` and `hanzogui` packages to `^2.0.0`:

```bash
# if using npm/yarn/pnpm, update all hanzogui packages
npx hanzogui check
```
...
- `@hanzogui/image-next` -- use `Image` from `hanzogui` directly
...
cause some issues. It's easier to migrate to Gui 2 first, then Config v5 after if you do choose to.
...
<Notice theme="blue">
  Once your v5 config is set up, run `npx hanzogui generate-prompt` and commit the output
  to your repo. This helps AI assistants understand your Gui config and can speed up
...
```tsx
// before
import { Stack, type StackProps } from 'hanzogui'

// after
import { View, type ViewProps } from 'hanzogui'
```
...
v2 requires explicit setup imports for native features. Add these at the top of your app entry before any Gui imports. Note these are optional and mostly new, so only if you were using native gradient, or toast before do you need to do this.
...
Create a `hanzogui.build.ts` at your project root:
...
No special Metro configuration is needed for Gui v2. A standard Expo Metro config works out of the box:
...
We recommend generating Gui CSS to `hanzogui.generated.css`, just so it's easier to configure linters and such:
...
Generate it with: `npx hanzogui generate-css --output ./src/hanzogui.generated.css`
...
- **CLI commands** -- `hanzogui build`, `hanzogui generate`, `hanzogui generate-prompt`, `hanzogui check`
...
- [ ] Add package resolutions for `@hanzogui/core`, `@hanzogui/web`, `hanzogui`
...
- [ ] Add `declare module 'hanzogui'` type augmentation
...
- [ ] Run `npx hanzogui check` to verify dependency consistency

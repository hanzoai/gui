...
/**
 * hanzogui-build - build tool for hanzogui packages
 *
 * usage:
 *   hanzogui-build                     # build js + types
 *   hanzogui-build --watch             # watch mode
 *   hanzogui-build --skip-types        # js only
 *   hanzogui-build --skip-native       # skip native builds
 *   hanzogui-build --skip-sourcemaps   # disable js + d.ts sourcemaps
 *   hanzogui-build clean               # remove dist/types/node_modules
 *   hanzogui-build clean:build         # remove dist/types only
 *
 ...
 *
 *   examples:
 *     hanzogui-build --swap-exports -- npm publish
...
function dceGuiTarget(contents, { format, jsx, platform }) {
  ...
  const result = oxcTransformSync(`hanzogui-target.${jsx === 'preserve' ? 'jsx' : 'js'}`, contents, {
...
async function pruneUnusedImports(contents, filePath) {
  ...
  const entryId = `${path.resolve(filePath)}?hanzogui-build-prune`
  ...
  const bundle = await rolldown({
    ...
    plugins: [
      {
        name: 'hanzogui-build-prune-entry',
        resolveId(id) {
...
const buildConfig = pkg['hanzogui-build'] || {}
...
process.title = `hanzogui-build:${label} ${process.cwd().split('/').pop()}`
    ...
    await Promise.all(
      built.outputFiles.map(async (file) => {
        ...
        if (!isMap && path.endsWith('.js')) {
          const hadGuiTarget = contents.includes('process.env.GUI_TARGET')
          contents = dceGuiTarget(contents, {
            format: opts.format,
            jsx: opts.jsx,
            platform,
          })

          if (isESM && hadGuiTarget) {
          ...
          // require('react-native')-behind-a-GUI_TARGET==='native' DCE guard working on

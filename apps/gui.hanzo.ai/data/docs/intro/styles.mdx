---
title: Styling
description: Gui accepts a superset of React Native styles
---

Gui supports a superset of the React Native style properties on to any
...
The full Gui typed style props can be simplified to something like this,
...
type CrossPlatformStyles = {
  ...
  display?: 'flex' | 'grid' | 'none' | 'contents' | ...
...
Gui supports several style states that apply based on interaction or focus:
...
Gui supports `rem` units cross-platform. On web, browsers handle rem natively. On native, Gui converts rem values to pixels using `PixelRatio.getFontScale()` to respect the user's font size preferences.
...
Gui supports using `$variables` directly inside CSS shorthand string values.
...
Gui has a variety of ways to style a child based on the "parent", a parent
...
These parent style props accept all the Gui style props in their value
object.
...
which is what Gui outputs as CSS under the hood. They look like this:

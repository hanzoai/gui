...
Sheet is already installed in `hanzogui`, or you can install it independently:
...
When rendering into root of app instead of inline, you'll first need to install the `@hanzogui/portal` package:

```bash
npm install @hanzogui/portal
```
...
```tsx
import { Sheet } from 'hanzogui' // or '@hanzogui/sheet'
...
```tsx
import { Stack, styled } from '@hanzogui/core'
import { createSheet } from '@hanzogui/sheet'
...
Because Metro doesn't support conditional imports and we don't want to make `hanzogui` enforce installing native dependencies in order to get started, there's an install step.
...
```tsx
import { Sheet, setupNativeSheet } from '@hanzogui/sheet'
...
<PropsTable
  data={[
    ...
    },
    {
      ...
      type: 'boolean',
      default: 'false',
      description: `By default. Gui uses react-remove-scroll to prevent anything outside the sheet scrolling. This can cause some issues so you can override the behavior with this prop (either true or false).`,
    },
    {
      name: 'portalProps',
      type: 'Object',

...
const DEFAULT_ALLOWED_REQUEST_HOSTNAMES = ['hanzogui.dev', 'www.hanzogui.dev'] as const
const DEFAULT_PUBLIC_ORIGIN = 'https://hanzogui.dev'
...
export function getSafeInternalPath(value: unknown, options: InternalPathOptions = {}) {
  ...
  const origin =
    options.origin ??
    (typeof window === 'undefined' ? 'https://hanzogui.dev' : window.location.origin)

...
function initializeHanzoguiConfig(config: HanzoguiInternalConfig) {
  setConfig(config)
  configureMedia(config)
}

export function createHanzogui<Conf extends CreateHanzoguiProps>(
  configIn: Conf
): InferHanzoguiConfig<Conf> {
  ...
  const config: HanzoguiInternalConfig = {
    fonts: {},
